...
extern bool pal_load_pvr_image_asset_header_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER *image_asset_header, size_t *image_asset_data_offset);

extern bool pal_load_pvr_image_asset_data_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER const *image_asset_header, size_t image_asset_data_offset, void *staging_buffer_base, size_t subresource_count, PAL_LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST const *subresource_memcpy_dests);
...
extern bool pal_load_pvr_image_asset_header_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER *image_asset_header, size_t *image_asset_data_offset)
{
    ...
    if (1U == header.depth)
    {
        if (header.height > 1U)
        {
            image_asset_header->type = PAL_ASSET_IMAGE_TYPE_2D;
        }
        else if (1U == header.height)
        {
            image_asset_header->type = PAL_ASSET_IMAGE_TYPE_1D;
        }
        else
        {
            return false;
    ...
    {
        image_asset_header->type = PAL_ASSET_IMAGE_TYPE_3D;
    }
    else
    {
        return false;
    ...
    if (0U == Pvr_GetPixelFormatPartHigh(header.pixelFormat))
    {
        ...
        // {
        //     image_asset_header->format = (Pvr_ChannelType_UnsignedFloat == header.channelType) ? PAL_ASSET_IMAGE_FORMAT_BC6H_UFLOAT_BLOCK : ((Pvr_ChannelType_SignedFloat == header.colorSpace) ? PAL_ASSET_IMAGE_FORMAT_BC6H_SFLOAT_BLOCK : static_cast<PAL_ASSET_IMAGE_FORMAT>(-1));
        // }
        // break;
        case Pvr_PixelTypeID_BC7:
        {
            image_asset_header->format = (Pvr_ColorSpace_lRGB == header.colorSpace) ? PAL_ASSET_IMAGE_FORMAT_BC7_UNORM_BLOCK : ((Pvr_ColorSpace_sRGB == header.colorSpace) ? static_cast<PAL_ASSET_IMAGE_FORMAT>(-1) : static_cast<PAL_ASSET_IMAGE_FORMAT>(-1));
        }
        break;
        case Pvr_PixelTypeID_ASTC_4x4:
        {
            image_asset_header->format = (Pvr_ColorSpace_lRGB == header.colorSpace) ? PAL_ASSET_IMAGE_FORMAT_ASTC_4x4_UNORM_BLOCK : ((Pvr_ColorSpace_sRGB == header.colorSpace) ? static_cast<PAL_ASSET_IMAGE_FORMAT>(-1) : static_cast<PAL_ASSET_IMAGE_FORMAT>(-1));
        }
        break;
        default:
    ...
    else
    {
        ...
        {
            image_asset_header->format = (Pvr_ChannelType_UnsignedByteNorm == header.channelType) ? ((Pvr_ColorSpace_lRGB == header.colorSpace) ? PAL_ASSET_IMAGE_FORMAT_R8G8B8A8_UNORM : static_cast<PAL_ASSET_IMAGE_FORMAT>(-1)) : static_cast<PAL_ASSET_IMAGE_FORMAT>(-1);
        }
        break;
        default:
            // TODO: support more format
...
{
#ifndef NDEBUG
    PAL_LOAD_IMAGE_ASSET_HEADER image_asset_header_for_validate;
    size_t image_asset_data_offset_for_validate;
    if (!pal_load_pvr_image_asset_header_from_input_stream(input_stream, &image_asset_header_for_validate, &image_asset_data_offset_for_validate))
    {
        return false;
    }
    ...
    for (uint32_t mipMap = 0; mipMap < image_asset_header->mip_levels; ++mipMap)
    {
        ...
        for (uint32_t arrayIndex = 0U; arrayIndex < image_asset_header->array_layers; ++arrayIndex)
        {
            ...
            {
                uint32_t dstSubresource = pal_load_image_asset_calculate_subresource_index(mipMap, arrayIndex, planeIndex, image_asset_header->mip_levels, image_asset_header->array_layers);
