...
void pal_d3d12_graphics_pipeline::init(ID3D12Device *device, pal_render_pass const *wrapped_render_pass, pal_pipeline_layout const *wrapped_pipeline_layout, size_t vertex_shader_module_code_size, void const *vertex_shader_module_code, size_t fragment_shader_module_code_size, void const *fragment_shader_module_code, uint32_t vertex_binding_count, PAL_GRAPHICS_PIPELINE_VERTEX_BINDING const *vertex_bindings, uint32_t vertex_attribute_count, PAL_GRAPHICS_PIPELINE_VERTEX_ATTRIBUTE const *vertex_attributes, bool depth_enable, PAL_GRAPHICS_PIPELINE_COMPARE_OPERATION wrapped_depth_compare_operation)
{
	...
	uint32_t const color_attachment_count = static_cast<pal_d3d12_render_pass const *>(wrapped_render_pass)->get_color_attachment_count();
	PAL_COLOR_ATTACHMENT_IMAGE_FORMAT const *const color_attachment_formats = static_cast<pal_d3d12_render_pass const *>(wrapped_render_pass)->get_color_attachment_formats();
	PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT const *const depth_stencil_attachment_format = static_cast<pal_d3d12_render_pass const *>(wrapped_render_pass)->get_depth_stencil_attachment_format();
	ID3D12RootSignature *const root_signature = static_cast<pal_d3d12_pipeline_layout const *>(wrapped_pipeline_layout)->get_root_signature();

	pal_vector<D3D12_INPUT_ELEMENT_DESC> input_element_descs(static_cast<size_t>(vertex_attribute_count));
	for (uint32_t vertex_attribute_index = 0U; vertex_attribute_index < vertex_attribute_count; ++vertex_attribute_index)
	{
		...
		case PAL_GRAPHICS_PIPELINE_VERTEX_ATTRIBUTE_FORMAT_R32G32B32_SFLOAT:
			...
			break;
		case PAL_GRAPHICS_PIPELINE_VERTEX_ATTRIBUTE_FORMAT_R16G16_UNORM:
	...
	{
	case PAL_GRAPHICS_PIPELINE_COMPARE_OPERATION_GREATER:
		depth_func = D3D12_COMPARISON_FUNC_GREATER;
		break;
	case PAL_GRAPHICS_PIPELINE_COMPARE_OPERATION_LESS:
		depth_func = D3D12_COMPARISON_FUNC_LESS;
		break;
	case PAL_GRAPHICS_PIPELINE_COMPARE_OPERATION_ALWAYS:
...
void pal_d3d12_graphics_pipeline::uninit()
{
	assert(NULL != this->m_pipeline_state);
...
pal_d3d12_graphics_pipeline::~pal_d3d12_graphics_pipeline()
{
	assert(NULL == this->m_pipeline_state);
}

uint32_t pal_d3d12_graphics_pipeline::get_vertex_buffer_count() const
{
	return static_cast<uint32_t>(this->m_vertex_buffer_strides.size());
}

uint32_t const *pal_d3d12_graphics_pipeline::get_vertex_buffer_strides() const
{
...
D3D12_PRIMITIVE_TOPOLOGY pal_d3d12_graphics_pipeline::get_primitive_topology() const
{
	return this->m_primitive_topology;
}

ID3D12PipelineState *pal_d3d12_graphics_pipeline::get_pipeline() const
{
	return this->m_pipeline_state;
}

pal_d3d12_compute_pipeline::pal_d3d12_compute_pipeline() : m_pipeline_state(NULL)
{
}

void pal_d3d12_compute_pipeline::init(ID3D12Device *device, pal_pipeline_layout const *wrapped_pipeline_layout, size_t compute_shader_module_code_size, void const *compute_shader_module_code)
{
	assert(NULL != wrapped_pipeline_layout);
	ID3D12RootSignature *const root_signature = static_cast<pal_d3d12_pipeline_layout const *>(wrapped_pipeline_layout)->get_root_signature();
...
void pal_d3d12_compute_pipeline::uninit()
{
	assert(NULL != this->m_pipeline_state);
...
pal_d3d12_compute_pipeline::~pal_d3d12_compute_pipeline()
{
	assert(NULL == this->m_pipeline_state);
}

ID3D12PipelineState *pal_d3d12_compute_pipeline::get_pipeline() const
{
	return this->m_pipeline_state;
}
