...
pal_d3d12_concrete_color_attachment_image::pal_d3d12_concrete_color_attachment_image() : m_heap(NULL), m_resource(NULL), m_render_target_view_descriptor_heap(NULL), m_render_target_view_descriptor{0U}
{
}

void pal_d3d12_concrete_color_attachment_image::init(ID3D12Device *device, bool uma, PAL_COLOR_ATTACHMENT_IMAGE_FORMAT wrapped_color_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
...
void pal_d3d12_concrete_color_attachment_image::uninit()
{
	assert(NULL != this->m_render_target_view_descriptor_heap);
...
pal_d3d12_concrete_color_attachment_image::~pal_d3d12_concrete_color_attachment_image()
{
	assert(NULL == this->m_render_target_view_descriptor_heap);
	assert(NULL == this->m_resource);
	assert(NULL == this->m_heap);
...
ID3D12Resource *pal_d3d12_concrete_color_attachment_image::get_resource() const
{
	return this->m_resource;
}

D3D12_CPU_DESCRIPTOR_HANDLE pal_d3d12_concrete_color_attachment_image::get_render_target_view_descriptor() const
{
	return this->m_render_target_view_descriptor;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *pal_d3d12_concrete_color_attachment_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

pal_sampled_image const *pal_d3d12_concrete_color_attachment_image::get_sampled_image() const
{
	return static_cast<pal_d3d12_sampled_image const *>(this);
}

pal_d3d12_concrete_depth_stencil_attachment_image::pal_d3d12_concrete_depth_stencil_attachment_image() : m_heap(NULL), m_resource(NULL), m_depth_stencil_view_descriptor_heap(NULL), m_depth_stencil_view_descriptor{0U}
{
}

void pal_d3d12_concrete_depth_stencil_attachment_image::init(ID3D12Device *device, bool uma, PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT wrapped_depth_stencil_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
...
void pal_d3d12_concrete_depth_stencil_attachment_image::uninit()
{
	assert(NULL != this->m_depth_stencil_view_descriptor_heap);
...
pal_d3d12_concrete_depth_stencil_attachment_image::~pal_d3d12_concrete_depth_stencil_attachment_image()
{
	assert(NULL == this->m_depth_stencil_view_descriptor_heap);
	assert(NULL == this->m_resource);
	assert(NULL == this->m_heap);
...
ID3D12Resource *pal_d3d12_concrete_depth_stencil_attachment_image::get_resource() const
{
	return this->m_resource;
}

D3D12_CPU_DESCRIPTOR_HANDLE pal_d3d12_concrete_depth_stencil_attachment_image::get_depth_stencil_view_descriptor() const
{
	return this->m_depth_stencil_view_descriptor;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *pal_d3d12_concrete_depth_stencil_attachment_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

pal_sampled_image const *pal_d3d12_concrete_depth_stencil_attachment_image::get_sampled_image() const
{
	return static_cast<pal_d3d12_sampled_image const *>(this);
}

pal_d3d12_concrete_storage_image::pal_d3d12_concrete_storage_image() : m_heap(NULL), m_resource(NULL)
{
}

void pal_d3d12_concrete_storage_image::init(ID3D12Device *device, bool uma, PAL_STORAGE_IMAGE_FORMAT wrapped_storage_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	DXGI_FORMAT unwrapped_format;
	switch (wrapped_storage_image_format)
	{
	case PAL_STORAGE_IMAGE_FORMAT_R16_SFLOAT:
		unwrapped_format = DXGI_FORMAT_R16_FLOAT;
		break;
	case PAL_STORAGE_IMAGE_FORMAT_R16G16B16A16_SFLOAT:
	...
	assert(NULL == this->m_resource);
	{
		...
		HRESULT const hr_create_placed_resource = device->CreatePlacedResource(this->m_heap, 0U, &resource_desc, (!allow_sampled_image) ? D3D12_RESOURCE_STATE_UNORDERED_ACCESS : (D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE), NULL, IID_PPV_ARGS(&this->m_resource));
		assert(SUCCEEDED(hr_create_placed_resource));
	}
...
void pal_d3d12_concrete_storage_image::uninit()
{
	assert(NULL != this->m_resource);
	this->m_resource->Release();
	this->m_resource = NULL;
...
pal_d3d12_concrete_storage_image::~pal_d3d12_concrete_storage_image()
{
	assert(NULL == this->m_resource);
	assert(NULL == this->m_heap);
}

ID3D12Resource *pal_d3d12_concrete_storage_image::get_resource() const
{
	return this->m_resource;
}

D3D12_UNORDERED_ACCESS_VIEW_DESC const *pal_d3d12_concrete_storage_image::get_unordered_access_view_desc() const
{
	return &this->m_unordered_access_view_desc;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *pal_d3d12_concrete_storage_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

pal_sampled_image const *pal_d3d12_concrete_storage_image::get_sampled_image() const
{
	return static_cast<pal_d3d12_sampled_image const *>(this);
}

pal_d3d12_asset_sampled_image::pal_d3d12_asset_sampled_image(ID3D12Resource *resource, D3D12MA::Allocation *allocation, D3D12_SHADER_RESOURCE_VIEW_DESC const *shader_resource_view_desc) : m_resource(resource), m_allocation(allocation), m_shader_resource_view_desc(*shader_resource_view_desc)
{
}

void pal_d3d12_asset_sampled_image::steal(ID3D12Resource **out_resource, D3D12MA::Allocation **out_allocation)
{
	assert(NULL != out_resource);
	assert(NULL != out_allocation);

	(*out_resource) = this->m_resource;
	(*out_allocation) = this->m_allocation;

	this->m_resource = NULL;
	this->m_allocation = NULL;
}

pal_d3d12_asset_sampled_image::~pal_d3d12_asset_sampled_image()
{
	assert(NULL == this->m_resource);
	assert(NULL == this->m_allocation);
}

ID3D12Resource *pal_d3d12_asset_sampled_image::get_resource() const
{
	return this->m_resource;
}

D3D12_SHADER_RESOURCE_VIEW_DESC const *pal_d3d12_asset_sampled_image::get_shader_resource_view_desc() const
{
	return &this->m_shader_resource_view_desc;
}

pal_sampled_image const *pal_d3d12_asset_sampled_image::get_sampled_image() const
{
	return static_cast<pal_d3d12_sampled_image const *>(this);
}
