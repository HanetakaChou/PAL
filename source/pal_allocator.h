...
#ifndef _PAL_ALLOCATOR_H_
#define _PAL_ALLOCATOR_H_ 1
...
public:
    ...
    struct rebind
    {
        typedef pal_allocator<U> other;
    };

    pal_allocator() throw() {}
    pal_allocator(const pal_allocator &) throw() {}
    template <typename U>
    pal_allocator(const pal_allocator<U> &) throw() {}
    ...
    {
        pointer p = static_cast<pointer>(pal_malloc(sizeof(value_type) * n, alignof(value_type)));
        return p;
    }
...
inline bool operator==(const pal_allocator<T> &, const pal_allocator<U> &)
{
    return true;
}
