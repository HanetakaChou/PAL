...
void pal_vk_graphics_pipeline::init(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, pal_render_pass const *wrapped_render_pass, pal_pipeline_layout const *wrapped_pipeline_layout, size_t vertex_shader_module_code_size, void const *vertex_shader_module_code, size_t fragment_shader_module_code_size, void const *fragment_shader_module_code, uint32_t vertex_binding_count, PAL_GRAPHICS_PIPELINE_VERTEX_BINDING const *vertex_bindings, uint32_t vertex_attribute_count, PAL_GRAPHICS_PIPELINE_VERTEX_ATTRIBUTE const *vertex_attributes, bool depth_enable, PAL_GRAPHICS_PIPELINE_COMPARE_OPERATION wrapped_depth_compare_operation)
{
	...
	VkRenderPass render_pass = static_cast<pal_vk_render_pass const *>(wrapped_render_pass)->get_render_pass();

	assert(NULL != wrapped_pipeline_layout);
	VkPipelineLayout pipeline_layout = static_cast<pal_vk_pipeline_layout const *>(wrapped_pipeline_layout)->get_pipeline_layout();
	...
	pal_vector<VkVertexInputBindingDescription> vertex_binding_descriptions(static_cast<size_t>(vertex_binding_count));
	...
	pal_vector<VkVertexInputAttributeDescription> vertex_attribute_descriptions(static_cast<size_t>(vertex_attribute_count));
...
void pal_vk_graphics_pipeline::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
...
pal_vk_graphics_pipeline::~pal_vk_graphics_pipeline()
{
	assert(VK_NULL_HANDLE == this->m_pipeline);
}

VkPipeline pal_vk_graphics_pipeline::get_pipeline() const
{
	return this->m_pipeline;
}

pal_vk_compute_pipeline::pal_vk_compute_pipeline() : m_pipeline(VK_NULL_HANDLE)
{
}

void pal_vk_compute_pipeline::init(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, pal_pipeline_layout const *wrapped_pipeline_layout, size_t compute_shader_module_code_size, void const *compute_shader_module_code)
{
	PFN_vkCreateShaderModule const pfn_create_shader_module = reinterpret_cast<PFN_vkCreateShaderModule>(pfn_get_device_proc_addr(device, "vkCreateShaderModule"));
	assert(pfn_create_shader_module);
	PFN_vkDestroyShaderModule const pfn_destroy_shader_module = reinterpret_cast<PFN_vkDestroyShaderModule>(pfn_get_device_proc_addr(device, "vkDestroyShaderModule"));
...
void pal_vk_compute_pipeline::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
	PFN_vkDestroyPipeline const pfn_destroy_pipeline = reinterpret_cast<PFN_vkDestroyPipeline>(pfn_get_device_proc_addr(device, "vkDestroyPipeline"));
	assert(NULL != pfn_destroy_pipeline);
...
pal_vk_compute_pipeline::~pal_vk_compute_pipeline()
{
	assert(VK_NULL_HANDLE == this->m_pipeline);
}

VkPipeline pal_vk_compute_pipeline::get_pipeline() const
{
	return this->m_pipeline;
}
