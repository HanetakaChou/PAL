...
#ifndef NDEBUG
static VkBool32 VKAPI_PTR __internal_debug_utils_messenger_callback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity, VkDebugUtilsMessageTypeFlagsEXT messageTypes, const VkDebugUtilsMessengerCallbackDataEXT *pCallbackData, void *pUserData);
#endif
...
{
	void *new_unwrapped_device_base = pal_malloc(sizeof(pal_vk_device), alignof(pal_vk_device));
	assert(NULL != new_unwrapped_device_base);

	pal_vk_device *new_unwrapped_device = new (new_unwrapped_device_base) pal_vk_device{};
	new_unwrapped_device->init(support_ray_tracing);
	return new_unwrapped_device;
}

pal_vk_device::pal_vk_device()
	: m_support_ray_tracing(false),
	  ...
	  m_min_uniform_buffer_offset_alignment(-1),
	  m_min_storage_buffer_offset_alignment(-1),
	  m_optimal_buffer_copy_offset_alignment(-1),
	  m_optimal_buffer_copy_row_pitch_alignment(-1),
	  ...
	  m_upload_ring_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_staging_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_asset_vertex_position_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_asset_vertex_varying_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_asset_index_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  ...
	  m_storage_image_memory_index(VK_MAX_MEMORY_TYPES),
	  m_asset_sampled_image_memory_index(VK_MAX_MEMORY_TYPES),
	  m_scratch_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_asset_compacted_bottom_level_acceleration_structure_memory_index(VK_MAX_MEMORY_TYPES),
	  m_top_level_acceleration_structure_instance_buffer_memory_index(VK_MAX_MEMORY_TYPES),
	  m_top_level_acceleration_structure_memory_index(VK_MAX_MEMORY_TYPES),
	  ...
	  m_asset_allocator(VK_NULL_HANDLE){};

void pal_vk_device::init(bool support_ray_tracing)
{
...
#error Unknown Compiler
		...
		VkDebugUtilsMessengerCreateInfoEXT const debug_utils_messenger_create_info =
			{
				...
				__internal_debug_utils_messenger_callback,
				NULL};
	...
	assert(static_cast<uint32_t>(-1) == this->m_optimal_buffer_copy_row_pitch_alignment);
	{
		...
		pal_vector<VkPhysicalDevice> physical_devices(static_cast<size_t>(physical_device_count));
		...
		uint32_t first_discrete_gpu_physical_device_index = -1;
		VkDeviceSize first_discrete_gpu_min_uniform_buffer_offset_alignment = -1;
		VkDeviceSize first_discrete_gpu_min_storage_buffer_offset_alignment = -1;
		VkDeviceSize first_discrete_gpu_optimal_buffer_copy_offset_alignment = -1;
		VkDeviceSize first_discrete_gpu_optimal_buffer_copy_row_pitch_alignment = -1;
		uint32_t first_non_discrete_gpu_physical_device_index = -1;
		VkDeviceSize first_non_discrete_gpu_min_uniform_buffer_offset_alignment = -1;
		VkDeviceSize first_non_discrete_gpu_min_storage_buffer_offset_alignment = -1;
		VkDeviceSize first_non_discrete_gpu_optimal_buffer_copy_offset_alignment = -1;
		VkDeviceSize first_non_discrete_gpu_optimal_buffer_copy_row_pitch_alignment = -1;
	...
	uint32_t new_graphics_queue_queue_index = -1;
	uint32_t new_upload_queue_queue_index = -1;
	{
...
#error Unknown Compiler
		...
		pal_vector<VkQueueFamilyProperties> queue_family_properties(static_cast<size_t>(queue_family_property_count));
...
#error Unknown Compiler
	...
	assert(VK_NULL_HANDLE == this->m_device);
	{
		...
		VkPhysicalDeviceRayTracingPipelineFeaturesKHR const physical_device_ray_tracing_pipeline_features =
			{
		...
		VkPhysicalDeviceAccelerationStructureFeaturesKHR const physical_device_acceleration_structure_features = {
			...
			const_cast<VkPhysicalDeviceRayTracingPipelineFeaturesKHR *>(&physical_device_ray_tracing_pipeline_features),
			VK_TRUE,
			VK_FALSE,
			VK_FALSE,
			VK_FALSE,
		...
		void const *const device_create_info_next = (!this->m_support_ray_tracing) ? NULL : &physical_device_buffer_device_address_features;
	...
	assert(VK_MAX_MEMORY_TYPES == this->m_upload_ring_buffer_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_staging_buffer_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_asset_vertex_position_buffer_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_asset_vertex_varying_buffer_memory_index);
	...
	assert(VK_MAX_MEMORY_TYPES == this->m_storage_image_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_asset_sampled_image_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_scratch_buffer_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_asset_compacted_bottom_level_acceleration_structure_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_top_level_acceleration_structure_instance_buffer_memory_index);
	assert(VK_MAX_MEMORY_TYPES == this->m_top_level_acceleration_structure_memory_index);
	{
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_upload_ring_buffer_memory_index);
		{
			VkDeviceSize memory_requirements_size = -1;
			...
			this->m_upload_ring_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_upload_ring_buffer_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_upload_ring_buffer_memory_index);
		}
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_staging_buffer_memory_index);
		{
			...
			this->m_staging_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_staging_buffer_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_staging_buffer_memory_index);
		}
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_asset_vertex_position_buffer_memory_index);
		{
			...
			uint32_t memory_requirements_memory_type_bits = 0U;
			{
				VkBufferUsageFlags const usage = (!this->m_support_ray_tracing) ? (VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT) : (VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR);
			...
			this->m_asset_vertex_position_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_asset_vertex_position_buffer_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_asset_vertex_position_buffer_memory_index);
		}
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_asset_vertex_varying_buffer_memory_index);
		{
			...
			// NOT HOST_VISIBLE
			// The UMA driver may compress the buffer/texture to boost performance
			this->m_asset_vertex_varying_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_asset_vertex_varying_buffer_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_asset_vertex_varying_buffer_memory_index);
		}
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_asset_index_buffer_memory_index);
		{
			...
			uint32_t memory_requirements_memory_type_bits = 0U;
			{
				VkBufferUsageFlags const usage = (!this->m_support_ray_tracing) ? (VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT) : (VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT_KHR);
			...
			// NOT HOST_VISIBLE
			// The UMA driver may compress the buffer/texture to boost performance
			this->m_asset_index_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_asset_index_buffer_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_asset_index_buffer_memory_index);
		}
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_color_transient_attachment_image_memory_index);
		{
			...
			this->m_color_transient_attachment_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_color_transient_attachment_image_memory_index);
		...
		if (this->m_color_attachment_sampled_image_memory_index)
		{
			...
			// The lower index indicates the more performance
			this->m_color_attachment_sampled_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_color_attachment_sampled_image_memory_index);
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_depth_transient_attachment_image_memory_index);
		{
			...
			// The lower index indicates the more performance
			this->m_depth_transient_attachment_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_depth_transient_attachment_image_memory_index);
		...
		if (this->m_depth_attachment_image_format_support_sampled_image)
		{
			...
			// The lower index indicates the more performance
			this->m_depth_attachment_sampled_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_depth_attachment_sampled_image_memory_index);
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_depth_stencil_transient_attachment_image_memory_index);
		{
			...
			// The lower index indicates the more performance
			this->m_depth_stencil_transient_attachment_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_depth_stencil_transient_attachment_image_memory_index);
		...
		if (this->m_depth_stencil_attachment_image_format_support_sampled_image)
		{
			...
			// The lower index indicates the more performance
			this->m_depth_stencil_attachment_sampled_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_depth_stencil_attachment_sampled_image_memory_index);
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_storage_image_memory_index);
		if (this->m_storage_image_memory_index)
		{
			VkDeviceSize memory_requirements_size = VkDeviceSize(-1);
			uint32_t memory_requirements_memory_type_bits = 0U;
			{
			...
			// The lower index indicates the more performance
			this->m_storage_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_storage_image_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_storage_image_memory_index);
		}
		...
		assert(VK_MAX_MEMORY_TYPES == this->m_asset_sampled_image_memory_index);
		{
			...
			this->m_asset_sampled_image_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
			assert(VK_MAX_MEMORY_TYPES > this->m_asset_sampled_image_memory_index);
			assert(physical_device_memory_properties.memoryTypeCount > this->m_asset_sampled_image_memory_index);
		}
		...
		if (this->m_support_ray_tracing)
		{
			...
			assert(VK_MAX_MEMORY_TYPES == this->m_scratch_buffer_memory_index);
			{
				...
				this->m_scratch_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
				assert(VK_MAX_MEMORY_TYPES > this->m_scratch_buffer_memory_index);
				assert(physical_device_memory_properties.memoryTypeCount > this->m_scratch_buffer_memory_index);
			}
			...
			assert(VK_MAX_MEMORY_TYPES == this->m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index);
			{
				...
				this->m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
				assert(VK_MAX_MEMORY_TYPES > this->m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index);
				assert(physical_device_memory_properties.memoryTypeCount > this->m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index);
			}
			...
			assert(VK_MAX_MEMORY_TYPES == this->m_asset_compacted_bottom_level_acceleration_structure_memory_index);
			{
				...
				this->m_asset_compacted_bottom_level_acceleration_structure_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
				assert(VK_MAX_MEMORY_TYPES > this->m_asset_compacted_bottom_level_acceleration_structure_memory_index);
				assert(physical_device_memory_properties.memoryTypeCount > this->m_asset_compacted_bottom_level_acceleration_structure_memory_index);
			}
			...
			assert(VK_MAX_MEMORY_TYPES == this->m_top_level_acceleration_structure_instance_buffer_memory_index);
			{
				...
				this->m_top_level_acceleration_structure_instance_buffer_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
				assert(VK_MAX_MEMORY_TYPES > this->m_top_level_acceleration_structure_instance_buffer_memory_index);
				assert(physical_device_memory_properties.memoryTypeCount > this->m_top_level_acceleration_structure_instance_buffer_memory_index);
			}
			...
			assert(VK_MAX_MEMORY_TYPES == this->m_top_level_acceleration_structure_memory_index);
			{
				...
				this->m_top_level_acceleration_structure_memory_index = __internal_find_lowest_memory_type_index(&physical_device_memory_properties, memory_requirements_size, memory_requirements_memory_type_bits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
				assert(VK_MAX_MEMORY_TYPES > this->m_top_level_acceleration_structure_memory_index);
				assert(physical_device_memory_properties.memoryTypeCount > this->m_top_level_acceleration_structure_memory_index);
			}
		}
	}
	...
	assert(VK_NULL_HANDLE == this->m_asset_allocator);
	{
...
extern "C" void pal_destroy_vk_device(pal_device *wrapped_device)
{
	assert(NULL != wrapped_device);
	pal_vk_device *delete_unwrapped_device = static_cast<pal_vk_device *>(wrapped_device);

	delete_unwrapped_device->uninit();

	delete_unwrapped_device->~pal_vk_device();
	pal_free(delete_unwrapped_device);
}

void pal_vk_device::uninit()
{
	assert(NULL != this->m_pfn_get_instance_proc_addr);
	assert(VK_NULL_HANDLE != this->m_instance);
#ifndef NDEBUG
...
pal_graphics_queue *pal_vk_device::create_graphics_queue() const
{
	...
	void *new_pal_graphics_queue_base = pal_malloc(sizeof(pal_vk_graphics_queue), alignof(pal_vk_graphics_queue));
	assert(NULL != new_pal_graphics_queue_base);

	pal_vk_graphics_queue *new_pal_graphics_queue = new (new_pal_graphics_queue_base) pal_vk_graphics_queue{this->m_has_dedicated_upload_queue, this->m_upload_queue_family_index, this->m_graphics_queue_family_index, this->m_graphics_queue, pfn_queue_submit, pfn_queue_present};
	return new_pal_graphics_queue;
}

void pal_vk_device::destroy_graphics_queue(pal_graphics_queue *pal_graphics_queue) const
{
	assert(NULL != pal_graphics_queue);
	pal_vk_graphics_queue *delete_graphics_queue = static_cast<pal_vk_graphics_queue *>(pal_graphics_queue);

	VkQueue stealed_graphics_queue = VK_NULL_HANDLE;
	delete_graphics_queue->steal(&stealed_graphics_queue);

	delete_graphics_queue->~pal_vk_graphics_queue();
	pal_free(delete_graphics_queue);

	assert(stealed_graphics_queue == this->m_graphics_queue);
}

pal_upload_queue *pal_vk_device::create_upload_queue() const
{
	PFN_vkQueueSubmit pfn_queue_submit = reinterpret_cast<PFN_vkQueueSubmit>(this->m_pfn_get_device_proc_addr(this->m_device, "vkQueueSubmit"));
	assert(NULL != pfn_queue_submit);

	void *new_pal_upload_queue_base = pal_malloc(sizeof(pal_vk_upload_queue), alignof(pal_vk_upload_queue));
	assert(NULL != new_pal_upload_queue_base);

	pal_vk_upload_queue *new_pal_upload_queue = new (new_pal_upload_queue_base) pal_vk_upload_queue{this->m_has_dedicated_upload_queue, this->m_upload_queue_family_index, this->m_graphics_queue_family_index, this->m_upload_queue, pfn_queue_submit};
	return new_pal_upload_queue;
}

void pal_vk_device::destroy_upload_queue(pal_upload_queue *pal_upload_queue) const
{
	assert(NULL != pal_upload_queue);
	pal_vk_upload_queue *delete_upload_queue = static_cast<pal_vk_upload_queue *>(pal_upload_queue);

	VkQueue stealed_upload_queue = VK_NULL_HANDLE;
	delete_upload_queue->steal(&stealed_upload_queue);

	delete_upload_queue->~pal_vk_upload_queue();
	pal_free(delete_upload_queue);

	assert(stealed_upload_queue == this->m_upload_queue);
}

pal_graphics_command_buffer *pal_vk_device::create_graphics_command_buffer() const
{
	void *new_unwrapped_graphics_command_buffer_base = pal_malloc(sizeof(pal_vk_graphics_command_buffer), alignof(pal_vk_graphics_command_buffer));
	assert(NULL != new_unwrapped_graphics_command_buffer_base);

	pal_vk_graphics_command_buffer *new_unwrapped_graphics_command_buffer = new (new_unwrapped_graphics_command_buffer_base) pal_vk_graphics_command_buffer{};
...
void pal_vk_device::reset_graphics_command_buffer(pal_graphics_command_buffer *pal_graphics_command_buffer) const
{
	assert(NULL != pal_graphics_command_buffer);
	VkCommandPool command_pool = static_cast<pal_vk_graphics_command_buffer *>(pal_graphics_command_buffer)->get_command_pool();
...
void pal_vk_device::destroy_graphics_command_buffer(pal_graphics_command_buffer *wrapped_graphics_command_buffer) const
{
	assert(NULL != wrapped_graphics_command_buffer);
	pal_vk_graphics_command_buffer *delete_unwrapped_graphics_command_buffer = static_cast<pal_vk_graphics_command_buffer *>(wrapped_graphics_command_buffer);
	...
	delete_unwrapped_graphics_command_buffer->~pal_vk_graphics_command_buffer();
	pal_free(delete_unwrapped_graphics_command_buffer);
}

pal_upload_command_buffer *pal_vk_device::create_upload_command_buffer() const
{
	void *new_unwrapped_upload_command_buffer_base = pal_malloc(sizeof(pal_vk_upload_command_buffer), alignof(pal_vk_upload_command_buffer));
	assert(NULL != new_unwrapped_upload_command_buffer_base);

	pal_vk_upload_command_buffer *new_unwrapped_upload_command_buffer = new (new_unwrapped_upload_command_buffer_base) pal_vk_upload_command_buffer{};
...
void pal_vk_device::reset_upload_command_buffer(pal_upload_command_buffer *pal_upload_command_buffer) const
{
	assert(NULL != pal_upload_command_buffer);
	VkCommandPool upload_command_pool = static_cast<pal_vk_upload_command_buffer *>(pal_upload_command_buffer)->get_upload_command_pool();
	VkCommandPool graphics_command_pool = static_cast<pal_vk_upload_command_buffer *>(pal_upload_command_buffer)->get_graphics_command_pool();
...
void pal_vk_device::destroy_upload_command_buffer(pal_upload_command_buffer *wrapped_upload_command_buffer) const
{
	assert(NULL != wrapped_upload_command_buffer);
	pal_vk_upload_command_buffer *delete_unwrapped_upload_command_buffer = static_cast<pal_vk_upload_command_buffer *>(wrapped_upload_command_buffer);

	delete_unwrapped_upload_command_buffer->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_upload_command_buffer->~pal_vk_upload_command_buffer();
	pal_free(delete_unwrapped_upload_command_buffer);
}

pal_fence *pal_vk_device::create_fence(bool signaled) const
{
	...
	void *new_pal_fence_base = pal_malloc(sizeof(pal_vk_fence), alignof(pal_vk_fence));
	assert(NULL != new_pal_fence_base);

	pal_vk_fence *new_pal_fence = new (new_pal_fence_base) pal_vk_fence{new_fence};
	return new_pal_fence;
}

void pal_vk_device::wait_for_fence(pal_fence *pal_fence) const
{
	assert(NULL != pal_fence);
	VkFence fence = static_cast<pal_vk_fence *>(pal_fence)->get_fence();
...
void pal_vk_device::reset_fence(pal_fence *pal_fence) const
{
	assert(NULL != pal_fence);
	VkFence fence = static_cast<pal_vk_fence *>(pal_fence)->get_fence();
...
void pal_vk_device::destroy_fence(pal_fence *pal_fence) const
{
	assert(NULL != pal_fence);
	pal_vk_fence *delete_fence = static_cast<pal_vk_fence *>(pal_fence);

	VkFence stealed_fence = VK_NULL_HANDLE;
	delete_fence->steal(&stealed_fence);

	delete_fence->~pal_vk_fence();
...
pal_descriptor_set_layout *pal_vk_device::create_descriptor_set_layout(uint32_t descriptor_set_binding_count, PAL_DESCRIPTOR_SET_LAYOUT_BINDING const *descriptor_set_bindings) const
{
	void *new_unwrapped_descriptor_set_layout_base = pal_malloc(sizeof(pal_vk_descriptor_set_layout), alignof(pal_vk_descriptor_set_layout));
	assert(NULL != new_unwrapped_descriptor_set_layout_base);

	pal_vk_descriptor_set_layout *new_unwrapped_descriptor_set_layout = new (new_unwrapped_descriptor_set_layout_base) pal_vk_descriptor_set_layout{};
...
void pal_vk_device::destroy_descriptor_set_layout(pal_descriptor_set_layout *wrapped_descriptor_set_layout) const
{
	assert(NULL != wrapped_descriptor_set_layout);
	pal_vk_descriptor_set_layout *delete_unwrapped_descriptor_set_layout = static_cast<pal_vk_descriptor_set_layout *>(wrapped_descriptor_set_layout);

	delete_unwrapped_descriptor_set_layout->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_descriptor_set_layout->~pal_vk_descriptor_set_layout();
	pal_free(delete_unwrapped_descriptor_set_layout);
}

pal_pipeline_layout *pal_vk_device::create_pipeline_layout(uint32_t descriptor_set_layout_count, pal_descriptor_set_layout const *const *pal_descriptor_set_layouts) const
{
	VkPipelineLayout new_pipeline_layout = VK_NULL_HANDLE;
	{
		...
		{
			assert(NULL != (pal_descriptor_set_layouts[set_index]));
			descriptor_set_layouts[set_index] = static_cast<pal_vk_descriptor_set_layout const *>(pal_descriptor_set_layouts[set_index])->get_descriptor_set_layout();
		}
	...
	void *new_pal_pipeline_layout_base = pal_malloc(sizeof(pal_vk_pipeline_layout), alignof(pal_vk_pipeline_layout));
	assert(NULL != new_pal_pipeline_layout_base);

	pal_vk_pipeline_layout *new_pal_pipeline_layout = new (new_pal_pipeline_layout_base) pal_vk_pipeline_layout{new_pipeline_layout};
	return new_pal_pipeline_layout;
}

void pal_vk_device::destroy_pipeline_layout(pal_pipeline_layout *pal_pipeline_layout) const
{
	assert(NULL != pal_pipeline_layout);
	pal_vk_pipeline_layout *delete_pipeline_layout = static_cast<pal_vk_pipeline_layout *>(pal_pipeline_layout);

	VkPipelineLayout stealed_pipeline_layout = VK_NULL_HANDLE;
	delete_pipeline_layout->steal(&stealed_pipeline_layout);

	delete_pipeline_layout->~pal_vk_pipeline_layout();
	pal_free(delete_pipeline_layout);
...
pal_descriptor_set *pal_vk_device::create_descriptor_set(pal_descriptor_set_layout const *descriptor_set_layout)
{
	void *new_unwrapped_descriptor_set_base = pal_malloc(sizeof(pal_vk_descriptor_set), alignof(pal_vk_descriptor_set));
	assert(NULL != new_unwrapped_descriptor_set_base);

	pal_vk_descriptor_set *new_unwrapped_descriptor_set = new (new_unwrapped_descriptor_set_base) pal_vk_descriptor_set{};
...
void pal_vk_device::write_descriptor_set(pal_descriptor_set *wrapped_descriptor_set, PAL_DESCRIPTOR_TYPE descriptor_type, uint32_t dst_binding, uint32_t dst_array_element, uint32_t src_descriptor_count, pal_upload_ring_buffer const *const *src_dynamic_uniform_buffers, uint32_t const *src_dynamic_uniform_buffers_range, pal_sampled_image const *const *src_sampled_images, pal_sampler const *const *src_samplers, pal_storage_image const *const *src_storage_images, pal_top_level_acceleration_structure const *const *src_top_level_acceleration_structures) const
{
	assert(NULL != wrapped_descriptor_set);
	pal_vk_descriptor_set *const unwrapped_descriptor_set = static_cast<pal_vk_descriptor_set *>(wrapped_descriptor_set);

	unwrapped_descriptor_set->write_descriptor(this->m_pfn_get_device_proc_addr, this->m_device, descriptor_type, dst_binding, dst_array_element, src_descriptor_count, src_dynamic_uniform_buffers, src_dynamic_uniform_buffers_range, src_sampled_images, src_samplers, src_storage_images, src_top_level_acceleration_structures);
}

void pal_vk_device::destroy_descriptor_set(pal_descriptor_set *wrapped_descriptor_set)
{
	assert(NULL != wrapped_descriptor_set);
	pal_vk_descriptor_set *delete_unwrapped_descriptor_set = static_cast<pal_vk_descriptor_set *>(wrapped_descriptor_set);

	delete_unwrapped_descriptor_set->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_descriptor_set->~pal_vk_descriptor_set();
	pal_free(delete_unwrapped_descriptor_set);
}

pal_render_pass *pal_vk_device::create_render_pass(uint32_t color_attachment_count, PAL_RENDER_PASS_COLOR_ATTACHMENT const *color_attachments, PAL_RENDER_PASS_DEPTH_STENCIL_ATTACHMENT const *depth_stencil_attachment) const
{
	...
	VkRenderPass new_render_pass = VK_NULL_HANDLE;
	{
		...
		if (NULL != depth_stencil_attachment)
		{
			...
			{
			case PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_D32_SFLOAT:
				...
				break;
			case PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_X8_D24_UNORM_PACK32:
				attachments_description[color_attachment_count].format = VK_FORMAT_X8_D24_UNORM_PACK32;
				attachments_description[color_attachment_count].stencilLoadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
				attachments_description[color_attachment_count].stencilStoreOp = VK_ATTACHMENT_STORE_OP_DONT_CARE;
				break;
			case PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_D32_SFLOAT_S8_UINT:
				...
				break;
			case PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_D24_UNORM_S8_UINT:
	...
	void *new_pal_render_pass_base = pal_malloc(sizeof(pal_vk_render_pass), alignof(pal_vk_render_pass));
	assert(NULL != new_pal_render_pass_base);

	pal_vk_render_pass *new_pal_render_pass = new (new_pal_render_pass_base) pal_vk_render_pass{new_render_pass};
	return new_pal_render_pass;
}

void pal_vk_device::destroy_render_pass(pal_render_pass *pal_render_pass) const
{
	assert(NULL != pal_render_pass);
	pal_vk_render_pass *delete_render_pass = static_cast<pal_vk_render_pass *>(pal_render_pass);

	VkRenderPass stealed_render_pass = VK_NULL_HANDLE;
	delete_render_pass->steal(&stealed_render_pass);

	delete_render_pass->~pal_vk_render_pass();
...
pal_graphics_pipeline *pal_vk_device::create_graphics_pipeline(pal_render_pass const *render_pass, pal_pipeline_layout const *pipeline_layout, size_t vertex_shader_module_code_size, void const *vertex_shader_module_code, size_t fragment_shader_module_code_size, void const *fragment_shader_module_code, uint32_t vertex_binding_count, PAL_GRAPHICS_PIPELINE_VERTEX_BINDING const *vertex_bindings, uint32_t vertex_attribute_count, PAL_GRAPHICS_PIPELINE_VERTEX_ATTRIBUTE const *vertex_attributes, bool depth_enable, PAL_GRAPHICS_PIPELINE_COMPARE_OPERATION depth_compare_operation) const
{
	void *new_unwrapped_graphics_pipeline_base = pal_malloc(sizeof(pal_vk_graphics_pipeline), alignof(pal_vk_graphics_pipeline));
	assert(NULL != new_unwrapped_graphics_pipeline_base);

	pal_vk_graphics_pipeline *new_unwrapped_graphics_pipeline = new (new_unwrapped_graphics_pipeline_base) pal_vk_graphics_pipeline{};
...
void pal_vk_device::destroy_graphics_pipeline(pal_graphics_pipeline *wrapped_graphics_pipeline) const
{
	assert(NULL != wrapped_graphics_pipeline);
	pal_vk_graphics_pipeline *delete_unwrapped_graphics_pipeline = static_cast<pal_vk_graphics_pipeline *>(wrapped_graphics_pipeline);

	delete_unwrapped_graphics_pipeline->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_graphics_pipeline->~pal_vk_graphics_pipeline();
	pal_free(delete_unwrapped_graphics_pipeline);
}

pal_compute_pipeline *pal_vk_device::create_compute_pipeline(pal_pipeline_layout const *pipeline_layout, size_t compute_shader_module_code_size, void const *compute_shader_module_code) const
{
	void *new_unwrapped_compute_pipeline_base = pal_malloc(sizeof(pal_vk_compute_pipeline), alignof(pal_vk_compute_pipeline));
	assert(NULL != new_unwrapped_compute_pipeline_base);

	pal_vk_compute_pipeline *new_unwrapped_compute_pipeline = new (new_unwrapped_compute_pipeline_base) pal_vk_compute_pipeline{};
...
void pal_vk_device::destroy_compute_pipeline(pal_compute_pipeline *wrapped_compute_pipeline) const
{
	assert(NULL != wrapped_compute_pipeline);
	pal_vk_compute_pipeline *delete_unwrapped_compute_pipeline = static_cast<pal_vk_compute_pipeline *>(wrapped_compute_pipeline);

	delete_unwrapped_compute_pipeline->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_compute_pipeline->~pal_vk_compute_pipeline();
	pal_free(delete_unwrapped_compute_pipeline);
}

pal_frame_buffer *pal_vk_device::create_frame_buffer(pal_render_pass const *pal_render_pass, uint32_t width, uint32_t height, uint32_t color_attachment_count, pal_color_attachment_image const *const *color_attachments, pal_depth_stencil_attachment_image const *depth_stencil_attachment) const
{
	assert(NULL != pal_render_pass);
	VkRenderPass render_pass = static_cast<pal_vk_render_pass const *>(pal_render_pass)->get_render_pass();
	...
	VkFramebuffer new_frame_buffer = VK_NULL_HANDLE;
	{
		...
		for (uint32_t color_attachment_index = 0U; color_attachment_index < color_attachment_count; ++color_attachment_index)
		{
			...
			attachments[color_attachment_index] = static_cast<pal_vk_color_attachment_image const *>(color_attachments[color_attachment_index])->get_image_view();
		}

		if (NULL != depth_stencil_attachment)
		{
			attachments[color_attachment_count] = static_cast<pal_vk_depth_stencil_attachment_image const *>(depth_stencil_attachment)->get_image_view();
		}
	...
	void *new_pal_frame_buffer_base = pal_malloc(sizeof(pal_vk_frame_buffer), alignof(pal_vk_frame_buffer));
	assert(NULL != new_pal_frame_buffer_base);

	pal_vk_frame_buffer *new_pal_frame_buffer = new (new_pal_frame_buffer_base) pal_vk_frame_buffer{new_frame_buffer};
	return new_pal_frame_buffer;
}

void pal_vk_device::destroy_frame_buffer(pal_frame_buffer *pal_frame_buffer) const
{
	assert(NULL != pal_frame_buffer);
	pal_vk_frame_buffer *delete_frame_buffer = static_cast<pal_vk_frame_buffer *>(pal_frame_buffer);

	VkFramebuffer stealed_frame_buffer = VK_NULL_HANDLE;
	delete_frame_buffer->steal(&stealed_frame_buffer);

	delete_frame_buffer->~pal_vk_frame_buffer();
...
uint32_t pal_vk_device::get_upload_ring_buffer_offset_alignment() const
{
	return this->m_min_uniform_buffer_offset_alignment;
}

pal_upload_ring_buffer *pal_vk_device::create_upload_ring_buffer(uint32_t size) const
{
	void *new_unwrapped_upload_ring_buffer_base = pal_malloc(sizeof(pal_vk_upload_ring_buffer), alignof(pal_vk_upload_ring_buffer));
	assert(NULL != new_unwrapped_upload_ring_buffer_base);

	pal_vk_upload_ring_buffer *new_unwrapped_upload_ring_buffer = new (new_unwrapped_upload_ring_buffer_base) pal_vk_upload_ring_buffer{};
	new_unwrapped_upload_ring_buffer->init(this->m_upload_ring_buffer_memory_index, size, this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);
	return new_unwrapped_upload_ring_buffer;
}

void pal_vk_device::destroy_upload_ring_buffer(pal_upload_ring_buffer *wrapped_upload_ring_buffer) const
{
	assert(NULL != wrapped_upload_ring_buffer);
	pal_vk_upload_ring_buffer *delete_unwrapped_upload_ring_buffer = static_cast<pal_vk_upload_ring_buffer *>(wrapped_upload_ring_buffer);

	delete_unwrapped_upload_ring_buffer->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_upload_ring_buffer->~pal_vk_upload_ring_buffer();
	pal_free(delete_unwrapped_upload_ring_buffer);
}

uint32_t pal_vk_device::get_staging_buffer_offset_alignment() const
{
	return this->m_optimal_buffer_copy_offset_alignment;
}

uint32_t pal_vk_device::get_staging_buffer_row_pitch_alignment() const
{
	return this->m_optimal_buffer_copy_row_pitch_alignment;
}

pal_staging_buffer *pal_vk_device::create_staging_buffer(uint32_t size) const
{
	void *new_unwrapped_staging_buffer_base = pal_malloc(sizeof(pal_vk_staging_buffer), alignof(pal_vk_staging_buffer));
	assert(NULL != new_unwrapped_staging_buffer_base);

	pal_vk_staging_buffer *new_unwrapped_staging_buffer = new (new_unwrapped_staging_buffer_base) pal_vk_staging_buffer{};
	new_unwrapped_staging_buffer->init(this->m_staging_buffer_memory_index, size, this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);
	return new_unwrapped_staging_buffer;
}

void pal_vk_device::destroy_staging_buffer(pal_staging_buffer *wrapped_staging_buffer) const
{
	assert(NULL != wrapped_staging_buffer);
	pal_vk_staging_buffer *delete_unwrapped_staging_buffer = static_cast<pal_vk_staging_buffer *>(wrapped_staging_buffer);

	delete_unwrapped_staging_buffer->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_staging_buffer->~pal_vk_staging_buffer();
	pal_free(delete_unwrapped_staging_buffer);
}

pal_asset_vertex_position_buffer *pal_vk_device::create_asset_vertex_position_buffer(uint32_t size) const
{
	void *new_unwrapped_asset_vertex_position_buffer_base = pal_malloc(sizeof(pal_vk_asset_vertex_position_buffer), alignof(pal_vk_asset_vertex_position_buffer));
	assert(NULL != new_unwrapped_asset_vertex_position_buffer_base);

	pal_vk_asset_vertex_position_buffer *new_unwrapped_asset_vertex_position_buffer = new (new_unwrapped_asset_vertex_position_buffer_base) pal_vk_asset_vertex_position_buffer{};
	new_unwrapped_asset_vertex_position_buffer->init(this->m_support_ray_tracing, this->m_pfn_get_device_proc_addr, this->m_device, this->m_asset_allocator, this->m_asset_vertex_position_buffer_memory_index, size);
	return new_unwrapped_asset_vertex_position_buffer;
}

void pal_vk_device::destroy_asset_vertex_position_buffer(pal_asset_vertex_position_buffer *wrapped_asset_vertex_position_buffer) const
{
	assert(NULL != wrapped_asset_vertex_position_buffer);
	pal_vk_asset_vertex_position_buffer *delete_unwrapped_asset_vertex_position_buffer = static_cast<pal_vk_asset_vertex_position_buffer *>(wrapped_asset_vertex_position_buffer);

	delete_unwrapped_asset_vertex_position_buffer->uninit(this->m_asset_allocator);

	delete_unwrapped_asset_vertex_position_buffer->~pal_vk_asset_vertex_position_buffer();
	pal_free(delete_unwrapped_asset_vertex_position_buffer);
}

pal_asset_vertex_varying_buffer *pal_vk_device::create_asset_vertex_varying_buffer(uint32_t size) const
{
	void *new_unwrapped_asset_vertex_varying_buffer_base = pal_malloc(sizeof(pal_vk_asset_vertex_varying_buffer), alignof(pal_vk_asset_vertex_varying_buffer));
	assert(NULL != new_unwrapped_asset_vertex_varying_buffer_base);

	pal_vk_asset_vertex_varying_buffer *new_unwrapped_asset_vertex_varying_buffer = new (new_unwrapped_asset_vertex_varying_buffer_base) pal_vk_asset_vertex_varying_buffer{};
	new_unwrapped_asset_vertex_varying_buffer->init(this->m_support_ray_tracing, this->m_asset_allocator, this->m_asset_vertex_varying_buffer_memory_index, size);
	return new_unwrapped_asset_vertex_varying_buffer;
}

void pal_vk_device::destroy_asset_vertex_varying_buffer(pal_asset_vertex_varying_buffer *wrapped_asset_vertex_varying_buffer) const
{
	assert(NULL != wrapped_asset_vertex_varying_buffer);
	pal_vk_asset_vertex_varying_buffer *delete_unwrapped_asset_vertex_varying_buffer = static_cast<pal_vk_asset_vertex_varying_buffer *>(wrapped_asset_vertex_varying_buffer);

	delete_unwrapped_asset_vertex_varying_buffer->uninit(this->m_asset_allocator);

	delete_unwrapped_asset_vertex_varying_buffer->~pal_vk_asset_vertex_varying_buffer();
	pal_free(delete_unwrapped_asset_vertex_varying_buffer);
}

pal_asset_index_buffer *pal_vk_device::create_asset_index_buffer(uint32_t size) const
{
	void *new_unwrapped_asset_index_buffer_base = pal_malloc(sizeof(pal_vk_asset_index_buffer), alignof(pal_vk_asset_index_buffer));
	assert(NULL != new_unwrapped_asset_index_buffer_base);

	pal_vk_asset_index_buffer *new_unwrapped_asset_index_buffer = new (new_unwrapped_asset_index_buffer_base) pal_vk_asset_index_buffer{};
	new_unwrapped_asset_index_buffer->init(this->m_support_ray_tracing, this->m_pfn_get_device_proc_addr, this->m_device, this->m_asset_allocator, this->m_asset_index_buffer_memory_index, size);
	return new_unwrapped_asset_index_buffer;
}

void pal_vk_device::destroy_asset_index_buffer(pal_asset_index_buffer *wrapped_asset_index_buffer) const
{
	assert(NULL != wrapped_asset_index_buffer);
	pal_vk_asset_index_buffer *delete_unwrapped_asset_index_buffer = static_cast<pal_vk_asset_index_buffer *>(wrapped_asset_index_buffer);

	delete_unwrapped_asset_index_buffer->uninit(this->m_asset_allocator);

	delete_unwrapped_asset_index_buffer->~pal_vk_asset_index_buffer();
	pal_free(delete_unwrapped_asset_index_buffer);
}

pal_color_attachment_image *pal_vk_device::create_color_attachment_image(PAL_COLOR_ATTACHMENT_IMAGE_FORMAT wrapped_color_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image) const
{
	void *new_unwrapped_color_attachment_image_base = pal_malloc(sizeof(pal_vk_concrete_color_attachment_image), alignof(pal_vk_concrete_color_attachment_image));
	assert(NULL != new_unwrapped_color_attachment_image_base);

	pal_vk_concrete_color_attachment_image *new_unwrapped_color_attachment_image = new (new_unwrapped_color_attachment_image_base) pal_vk_concrete_color_attachment_image{};
...
void pal_vk_device::destroy_color_attachment_image(pal_color_attachment_image *wrapped_color_attachment_image) const
{
	assert(NULL != wrapped_color_attachment_image);
	pal_vk_concrete_color_attachment_image *delete_unwrapped_color_attachment_image = static_cast<pal_vk_concrete_color_attachment_image *>(wrapped_color_attachment_image);

	delete_unwrapped_color_attachment_image->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_color_attachment_image->~pal_vk_concrete_color_attachment_image();
	pal_free(delete_unwrapped_color_attachment_image);
}

PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT pal_vk_device::get_depth_attachment_image_format() const
{
	PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT pal_depth_stencil_attachment_image_format;

	switch (this->m_depth_attachment_image_format)
	{
	case VK_FORMAT_D32_SFLOAT:
		pal_depth_stencil_attachment_image_format = PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_D32_SFLOAT;
		break;
	case VK_FORMAT_X8_D24_UNORM_PACK32:
		pal_depth_stencil_attachment_image_format = PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_X8_D24_UNORM_PACK32;
		break;
	default:
		assert(false);
		pal_depth_stencil_attachment_image_format = static_cast<PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT>(-1);
	}

	return pal_depth_stencil_attachment_image_format;
}

PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT pal_vk_device::get_depth_stencil_attachment_image_format() const
{
	PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT pal_depth_stencil_attachment_image_format;
	...
	case VK_FORMAT_D32_SFLOAT_S8_UINT:
		pal_depth_stencil_attachment_image_format = PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_D32_SFLOAT_S8_UINT;
		break;
	case VK_FORMAT_D24_UNORM_S8_UINT:
		pal_depth_stencil_attachment_image_format = PAL_DEPTH_STENCIL_ATTACHMENT_FORMAT_D24_UNORM_S8_UINT;
		break;
	default:
		assert(false);
		pal_depth_stencil_attachment_image_format = static_cast<PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT>(-1);
...
pal_depth_stencil_attachment_image *pal_vk_device::create_depth_stencil_attachment_image(PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT wrapped_depth_stencil_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image) const
{
	void *new_unwrapped_depth_stencil_attachment_image_base = pal_malloc(sizeof(pal_vk_concrete_depth_stencil_attachment_image), alignof(pal_vk_concrete_depth_stencil_attachment_image));
	assert(NULL != new_unwrapped_depth_stencil_attachment_image_base);

	pal_vk_concrete_depth_stencil_attachment_image *new_unwrapped_depth_stencil_attachment_image = new (new_unwrapped_depth_stencil_attachment_image_base) pal_vk_concrete_depth_stencil_attachment_image{};
...
void pal_vk_device::destroy_depth_stencil_attachment_image(pal_depth_stencil_attachment_image *wrapped_depth_stencil_attachment_image) const
{
	assert(NULL != wrapped_depth_stencil_attachment_image);
	pal_vk_concrete_depth_stencil_attachment_image *delete_unwrapped_depth_stencil_attachment_image = static_cast<pal_vk_concrete_depth_stencil_attachment_image *>(wrapped_depth_stencil_attachment_image);
	...
	delete_unwrapped_depth_stencil_attachment_image->~pal_vk_concrete_depth_stencil_attachment_image();
	pal_free(delete_unwrapped_depth_stencil_attachment_image);
}

pal_storage_image *pal_vk_device::create_storage_image(PAL_STORAGE_IMAGE_FORMAT storage_image_format, uint32_t width, uint32_t height, bool allow_sampled_image) const
{
	void *new_unwrapped_storage_image_base = pal_malloc(sizeof(pal_vk_concrete_storage_image), alignof(pal_vk_concrete_storage_image));
	assert(NULL != new_unwrapped_storage_image_base);

	pal_vk_concrete_storage_image *new_unwrapped_storage_image = new (new_unwrapped_storage_image_base) pal_vk_concrete_storage_image{};
	new_unwrapped_storage_image->init(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, this->m_storage_image_memory_index, storage_image_format, width, height, allow_sampled_image);
	return new_unwrapped_storage_image;
}

void pal_vk_device::destroy_storage_image(pal_storage_image *wrapped_storage_image) const
{
	assert(NULL != wrapped_storage_image);
	pal_vk_concrete_storage_image *delete_unwrapped_storage_image = static_cast<pal_vk_concrete_storage_image *>(wrapped_storage_image);

	delete_unwrapped_storage_image->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_storage_image->~pal_vk_concrete_storage_image();
	pal_free(delete_unwrapped_storage_image);
}

pal_asset_sampled_image *pal_vk_device::create_asset_sampled_image(PAL_ASSET_IMAGE_FORMAT wrapped_asset_sampled_image_format, uint32_t width, uint32_t height, uint32_t mip_levels) const
{
	PFN_vkCreateImageView const pfn_create_image_view = reinterpret_cast<PFN_vkCreateImageView>(this->m_pfn_get_device_proc_addr(this->m_device, "vkCreateImageView"));
	...
	case PAL_ASSET_IMAGE_FORMAT_R8G8B8A8_UNORM:
		new_format = VK_FORMAT_R8G8B8A8_UNORM;
		break;
	case PAL_ASSET_IMAGE_FORMAT_BC7_UNORM_BLOCK:
		if (this->m_physical_device_feature_texture_compression_BC)
		{
			new_format = VK_FORMAT_BC7_UNORM_BLOCK;
		}
		else if (this->m_physical_device_feature_texture_compression_ASTC_LDR)
		{
			// TODO: the data is not compatiable between BC7 and ASTC 4x4
			new_format = VK_FORMAT_ASTC_4x4_UNORM_BLOCK;
		}
		else
		{
			new_format = VK_FORMAT_UNDEFINED;
		}
		break;
	default:
		assert(false);
	...
	void *new_pal_asset_sampled_image_base = pal_malloc(sizeof(pal_vk_asset_sampled_image), alignof(pal_vk_asset_sampled_image));
	assert(NULL != new_pal_asset_sampled_image_base);

	pal_vk_asset_sampled_image *new_pal_asset_sampled_image = new (new_pal_asset_sampled_image_base) pal_vk_asset_sampled_image{};

	new_pal_asset_sampled_image->init(this->m_asset_allocator, this->m_asset_sampled_image_memory_index, pfn_create_image_view, this->m_device, this->m_allocation_callbacks, new_format, width, height, mip_levels);

	return new_pal_asset_sampled_image;
}

void pal_vk_device::destroy_asset_sampled_image(pal_asset_sampled_image *wrapped_asset_sampled_image) const
{
	assert(NULL != wrapped_asset_sampled_image);
	pal_vk_asset_sampled_image *delete_unwrapped_asset_sampled_image = static_cast<pal_vk_asset_sampled_image *>(wrapped_asset_sampled_image);

	PFN_vkDestroyImageView const pfn_destroy_image_view = reinterpret_cast<PFN_vkDestroyImageView>(this->m_pfn_get_device_proc_addr(this->m_device, "vkDestroyImageView"));
	assert(NULL != pfn_destroy_image_view);

	delete_unwrapped_asset_sampled_image->uninit(this->m_asset_allocator, pfn_destroy_image_view, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_asset_sampled_image->~pal_vk_asset_sampled_image();
	pal_free(delete_unwrapped_asset_sampled_image);
}

pal_sampler *pal_vk_device::create_sampler(PAL_SAMPLER_FILTER wrapped_filter) const
{
	...
	void *new_pal_sampler_base = pal_malloc(sizeof(pal_vk_sampler), alignof(pal_vk_sampler));
	assert(NULL != new_pal_sampler_base);

	pal_vk_sampler *new_pal_sampler = new (new_pal_sampler_base) pal_vk_sampler{new_sampler};
	return new_pal_sampler;
}

void pal_vk_device::destroy_sampler(pal_sampler *pal_sampler) const
{
	assert(NULL != pal_sampler);
	pal_vk_sampler *delete_sampler = static_cast<pal_vk_sampler *>(pal_sampler);

	VkSampler stealed_sampler = VK_NULL_HANDLE;
	delete_sampler->steal(&stealed_sampler);

	delete_sampler->~pal_vk_sampler();
...
pal_surface *pal_vk_device::create_surface(void *window_void) const
{
...
#error Unknown Platform
	...
	void *new_pal_surface_base = pal_malloc(sizeof(pal_vk_surface), alignof(pal_vk_surface));
	assert(NULL != new_pal_surface_base);

	pal_vk_surface *new_pal_surface = new (new_pal_surface_base) pal_vk_surface{new_surface};
	return new_pal_surface;
}

void pal_vk_device::destroy_surface(pal_surface *pal_surface) const
{
	assert(NULL != pal_surface);
	pal_vk_surface *delete_surface = static_cast<pal_vk_surface *>(pal_surface);

	VkSurfaceKHR stealed_surface = VK_NULL_HANDLE;
	delete_surface->steal(&stealed_surface);

	delete_surface->~pal_vk_surface();
	pal_free(delete_surface);
...
pal_swap_chain *pal_vk_device::create_swap_chain(pal_surface *pal_surface) const
{
	assert(NULL != pal_surface);
	VkSurfaceKHR surface = static_cast<pal_vk_surface *>(pal_surface)->get_surface();
	...
	uint32_t request_swap_chain_image_count = -1;
	{
		...
		// Format
		{
			...
			VkSurfaceFormatKHR *surface_formats = static_cast<VkSurfaceFormatKHR *>(pal_malloc(sizeof(VkQueueFamilyProperties) * surface_format_count_1, alignof(VkQueueFamilyProperties)));
	...
	VkPresentModeKHR present_mode = VK_PRESENT_MODE_FIFO_KHR;
	{
		...
		pal_vector<VkPresentModeKHR> present_modes(static_cast<size_t>(present_mode_count));
	...
	uint32_t new_swap_chain_image_count = -1;
	VkImage *new_swap_chain_images = NULL;
	{
		...
		new_swap_chain_images = static_cast<VkImage *>(pal_malloc(sizeof(VkImage) * new_swap_chain_image_count, alignof(VkImage)));
	...
	pal_vk_swap_chain_image_view *new_swap_chain_image_views = NULL;
	{
		new_swap_chain_image_views = static_cast<pal_vk_swap_chain_image_view *>(pal_malloc(sizeof(pal_vk_swap_chain_image_view) * new_swap_chain_image_count, alignof(pal_vk_swap_chain_image_view)));
		...
		for (uint32_t swap_chain_image_index = 0U; swap_chain_image_index < new_swap_chain_image_count; ++swap_chain_image_index)
		{
			...
			new (new_swap_chain_image_views + swap_chain_image_index) pal_vk_swap_chain_image_view{new_image_view};
		}
	}

	assert(NULL != new_swap_chain_images);
	pal_free(new_swap_chain_images);
	new_swap_chain_images = NULL;

	void *new_pal_swap_chain_base = pal_malloc(sizeof(pal_vk_swap_chain), alignof(pal_vk_swap_chain));
	assert(NULL != new_pal_swap_chain_base);

	pal_vk_swap_chain *new_pal_swap_chain = new (new_pal_swap_chain_base) pal_vk_swap_chain{new_swap_chain, new_swap_chain_image_format, new_swap_chain_image_width, new_swap_chain_image_height, new_swap_chain_image_count, new_swap_chain_image_views};
	return new_pal_swap_chain;
}

bool pal_vk_device::acquire_next_image(pal_graphics_command_buffer *pal_graphics_command_buffer, pal_swap_chain const *pal_swap_chain, uint32_t *out_swap_chain_image_index) const
{
	assert(NULL != pal_graphics_command_buffer);
	assert(NULL != pal_swap_chain);
	assert(NULL != out_swap_chain_image_index);
	VkSemaphore acquire_next_image_semaphore = static_cast<pal_vk_graphics_command_buffer const *>(pal_graphics_command_buffer)->get_acquire_next_image_semaphore();
	VkSwapchainKHR swap_chain = static_cast<pal_vk_swap_chain const *>(pal_swap_chain)->get_swap_chain();
...
void pal_vk_device::destroy_swap_chain(pal_swap_chain *pal_swap_chain) const
{
	assert(NULL != pal_swap_chain);
	pal_vk_swap_chain *delete_swap_chain = static_cast<pal_vk_swap_chain *>(pal_swap_chain);

	VkSwapchainKHR stealed_swap_chain = VK_NULL_HANDLE;
	uint32_t stealed_swap_chain_image_count = -1;
	pal_vk_swap_chain_image_view *delete_swap_chain_image_views = NULL;
	delete_swap_chain->steal(&stealed_swap_chain, &stealed_swap_chain_image_count, &delete_swap_chain_image_views);

	delete_swap_chain->~pal_vk_swap_chain();
	...
	pal_free(delete_swap_chain_image_views);

	pfn_destroy_swapchain(this->m_device, stealed_swap_chain, this->m_allocation_callbacks);
}

uint32_t pal_vk_device::get_scratch_buffer_offset_alignment() const
{
	return 256U;
}

pal_scratch_buffer *pal_vk_device::create_scratch_buffer(uint32_t size) const
{
	void *new_unwrapped_scratch_buffer_base = pal_malloc(sizeof(pal_vk_scratch_buffer), alignof(pal_vk_scratch_buffer));
	assert(NULL != new_unwrapped_scratch_buffer_base);

	pal_vk_scratch_buffer *new_unwrapped_scratch_buffer = new (new_unwrapped_scratch_buffer_base) pal_vk_scratch_buffer{};
	new_unwrapped_scratch_buffer->init(this->m_scratch_buffer_memory_index, size, this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);
	return new_unwrapped_scratch_buffer;
}

void pal_vk_device::destroy_scratch_buffer(pal_scratch_buffer *wrapped_scratch_buffer) const
{
	assert(NULL != wrapped_scratch_buffer);
	pal_vk_scratch_buffer *delete_unwrapped_scratch_buffer = static_cast<pal_vk_scratch_buffer *>(wrapped_scratch_buffer);

	delete_unwrapped_scratch_buffer->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_scratch_buffer->~pal_vk_scratch_buffer();
	pal_free(delete_unwrapped_scratch_buffer);
}

uint32_t pal_vk_device::get_staging_non_compacted_bottom_level_acceleration_structure_buffer_offset_alignment() const
{
	return 256U;
}

pal_staging_non_compacted_bottom_level_acceleration_structure_buffer *pal_vk_device::create_staging_non_compacted_bottom_level_acceleration_structure_buffer(uint32_t size) const
{
	void *new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer_base = pal_malloc(sizeof(pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer), alignof(pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer));
	assert(NULL != new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer_base);

	pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer *new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer = new (new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer_base) pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer{};
	new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer->init(this->m_staging_non_compacted_bottom_level_acceleration_structure_buffer_memory_index, size, this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);
	return new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer;
}

void pal_vk_device::destroy_staging_non_compacted_bottom_level_acceleration_structure_buffer(pal_staging_non_compacted_bottom_level_acceleration_structure_buffer *wrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer) const
{
	assert(NULL != wrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer);
	pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer *delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer = static_cast<pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer *>(wrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer);

	delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer->~pal_vk_staging_non_compacted_bottom_level_acceleration_structure_buffer();
	pal_free(delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer);
}

void pal_vk_device::get_staging_non_compacted_bottom_level_acceleration_structure_size(uint32_t acceleration_structure_geometry_count, PAL_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const *wrapped_bottom_level_acceleration_structure_geometries, uint32_t *staging_non_compacted_bottom_level_acceleration_structure_size, uint32_t *build_scratch_size) const
{
	...
	pal_vector<VkAccelerationStructureGeometryKHR> acceleration_structure_geometries;
	pal_vector<uint32_t> max_primitive_counts;
	...
	{
		PAL_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const &wrapped_bottom_level_acceleration_structure_geometry = wrapped_bottom_level_acceleration_structure_geometries[acceleration_structure_geometry_index];
		...
		VkDeviceAddress const vertex_position_buffer_device_memory_range_base = static_cast<pal_vk_asset_vertex_position_buffer *>(wrapped_bottom_level_acceleration_structure_geometry.vertex_position_buffer)->get_device_memory_range_base();
		...
		VkDeviceAddress const index_buffer_device_memory_range_base = (PAL_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? static_cast<pal_vk_asset_index_buffer *>(wrapped_bottom_level_acceleration_structure_geometry.index_buffer)->get_device_memory_range_base() : NULL;
		...
		VkAccelerationStructureGeometryKHR const acceleration_structure_geometry = {
			...
			{.triangles =
				 {VK_STRUCTURE_TYPE_ACCELERATION_STRUCTURE_GEOMETRY_TRIANGLES_DATA_KHR,
				  ...
				  (PAL_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? (wrapped_bottom_level_acceleration_structure_geometry.index_count - 1U) : (wrapped_bottom_level_acceleration_structure_geometry.vertex_count - 1U),
				  index_type,
		...
		assert(0U == ((PAL_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? (wrapped_bottom_level_acceleration_structure_geometry.index_count % 3U) : (wrapped_bottom_level_acceleration_structure_geometry.vertex_count % 3U)));
		uint32_t const max_primitive_count = (PAL_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? (wrapped_bottom_level_acceleration_structure_geometry.index_count / 3U) : (wrapped_bottom_level_acceleration_structure_geometry.vertex_count / 3U);
...
pal_staging_non_compacted_bottom_level_acceleration_structure *pal_vk_device::create_staging_non_compacted_bottom_level_acceleration_structure(pal_staging_non_compacted_bottom_level_acceleration_structure_buffer const *wrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer, uint32_t offset, uint32_t size) const
{
	void *new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_base = pal_malloc(sizeof(pal_vk_staging_non_compacted_bottom_level_acceleration_structure), alignof(pal_vk_staging_non_compacted_bottom_level_acceleration_structure));
	assert(NULL != new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_base);

	pal_vk_staging_non_compacted_bottom_level_acceleration_structure *new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure = new (new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure_base) pal_vk_staging_non_compacted_bottom_level_acceleration_structure{};
	new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure->init(wrapped_staging_non_compacted_bottom_level_acceleration_structure_buffer, offset, size, this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);
	return new_unwrapped_staging_non_compacted_bottom_level_acceleration_structure;
}

void pal_vk_device::destroy_staging_non_compacted_bottom_level_acceleration_structure(pal_staging_non_compacted_bottom_level_acceleration_structure *wrapped_staging_non_compacted_bottom_level_acceleration_structure) const
{
	assert(NULL != wrapped_staging_non_compacted_bottom_level_acceleration_structure);
	pal_vk_staging_non_compacted_bottom_level_acceleration_structure *delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure = static_cast<pal_vk_staging_non_compacted_bottom_level_acceleration_structure *>(wrapped_staging_non_compacted_bottom_level_acceleration_structure);

	delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure->~pal_vk_staging_non_compacted_bottom_level_acceleration_structure();
	pal_free(delete_unwrapped_staging_non_compacted_bottom_level_acceleration_structure);
}

pal_compacted_bottom_level_acceleration_structure_size_query_pool *pal_vk_device::create_compacted_bottom_level_acceleration_structure_size_query_pool(uint32_t query_count) const
{
	void *new_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool_base = pal_malloc(sizeof(pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool), alignof(pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool));
	assert(NULL != new_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool_base);

	pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool *new_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool = new (new_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool_base) pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool{};
...
uint32_t pal_vk_device::get_compacted_bottom_level_acceleration_structure_size_query_pool_result(pal_compacted_bottom_level_acceleration_structure_size_query_pool const *wrapped_compacted_bottom_level_acceleration_structure_size_query_pool, uint32_t query_index) const
{
	assert(NULL != wrapped_compacted_bottom_level_acceleration_structure_size_query_pool);
	pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool const *const unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool = static_cast<pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool const *>(wrapped_compacted_bottom_level_acceleration_structure_size_query_pool);

	VkQueryPool const query_pool = unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool->get_query_pool();

	VkDeviceSize compacted_bottom_level_acceleration_structure_size = -1;
...
void pal_vk_device::destroy_compacted_bottom_level_acceleration_structure_size_query_pool(pal_compacted_bottom_level_acceleration_structure_size_query_pool *wrapped_compacted_bottom_level_acceleration_structure_size_query_pool) const
{
	assert(NULL != wrapped_compacted_bottom_level_acceleration_structure_size_query_pool);
	pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool *delete_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool = static_cast<pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool *>(wrapped_compacted_bottom_level_acceleration_structure_size_query_pool);
	...
	delete_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool->~pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool();
	pal_free(delete_unwrapped_compacted_bottom_level_acceleration_structure_size_query_pool);
}

pal_asset_compacted_bottom_level_acceleration_structure *pal_vk_device::create_asset_compacted_bottom_level_acceleration_structure(uint32_t size) const
{
	void *new_unwrapped_asset_compacted_bottom_level_acceleration_structure_base = pal_malloc(sizeof(pal_vk_asset_compacted_bottom_level_acceleration_structure), alignof(pal_vk_asset_compacted_bottom_level_acceleration_structure));
	assert(NULL != new_unwrapped_asset_compacted_bottom_level_acceleration_structure_base);

	pal_vk_asset_compacted_bottom_level_acceleration_structure *new_unwrapped_asset_compacted_bottom_level_acceleration_structure = new (new_unwrapped_asset_compacted_bottom_level_acceleration_structure_base) pal_vk_asset_compacted_bottom_level_acceleration_structure{};
	new_unwrapped_asset_compacted_bottom_level_acceleration_structure->init(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, this->m_asset_compacted_bottom_level_acceleration_structure_memory_index, this->m_asset_allocator, size);
	return new_unwrapped_asset_compacted_bottom_level_acceleration_structure;
}

void pal_vk_device::destroy_asset_compacted_bottom_level_acceleration_structure(pal_asset_compacted_bottom_level_acceleration_structure *wrapped_asset_compacted_bottom_level_acceleration_structure) const
{
	assert(NULL != wrapped_asset_compacted_bottom_level_acceleration_structure);
	pal_vk_asset_compacted_bottom_level_acceleration_structure *delete_unwrapped_asset_compacted_bottom_level_acceleration_structure = static_cast<pal_vk_asset_compacted_bottom_level_acceleration_structure *>(wrapped_asset_compacted_bottom_level_acceleration_structure);

	delete_unwrapped_asset_compacted_bottom_level_acceleration_structure->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, this->m_asset_allocator);

	delete_unwrapped_asset_compacted_bottom_level_acceleration_structure->~pal_vk_asset_compacted_bottom_level_acceleration_structure();
	pal_free(delete_unwrapped_asset_compacted_bottom_level_acceleration_structure);
}

pal_top_level_acceleration_structure_instance_buffer *pal_vk_device::create_top_level_acceleration_structure_instance_buffer(uint32_t instance_count) const
{
	void *new_unwrapped_top_level_acceleration_structure_instance_buffer_base = pal_malloc(sizeof(pal_vk_top_level_acceleration_structure_instance_buffer), alignof(pal_vk_top_level_acceleration_structure_instance_buffer));
	assert(NULL != new_unwrapped_top_level_acceleration_structure_instance_buffer_base);

	pal_vk_top_level_acceleration_structure_instance_buffer *new_unwrapped_top_level_acceleration_structure_instance_buffer = new (new_unwrapped_top_level_acceleration_structure_instance_buffer_base) pal_vk_top_level_acceleration_structure_instance_buffer{};
	new_unwrapped_top_level_acceleration_structure_instance_buffer->init(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, this->m_top_level_acceleration_structure_instance_buffer_memory_index, instance_count);
	return new_unwrapped_top_level_acceleration_structure_instance_buffer;
}

void pal_vk_device::write_top_level_acceleration_structure_instance_buffer(pal_top_level_acceleration_structure_instance_buffer *wrapped_top_level_acceleration_structure_instance_buffer, uint32_t instance_index, float const transform_matrix[3][4], bool force_closest_hit, bool force_any_hit, bool disable_back_face_cull, bool front_ccw, pal_asset_compacted_bottom_level_acceleration_structure *asset_compacted_bottom_level_acceleration_structure) const
{
	assert(NULL != wrapped_top_level_acceleration_structure_instance_buffer);
	pal_vk_top_level_acceleration_structure_instance_buffer *const unwrapped_top_level_acceleration_structure_instance_buffer = static_cast<pal_vk_top_level_acceleration_structure_instance_buffer *>(wrapped_top_level_acceleration_structure_instance_buffer);
...
void pal_vk_device::destroy_top_level_acceleration_structure_instance_buffer(pal_top_level_acceleration_structure_instance_buffer *wrapped_top_level_acceleration_structure_instance_buffer) const
{
	assert(NULL != wrapped_top_level_acceleration_structure_instance_buffer);
	pal_vk_top_level_acceleration_structure_instance_buffer *delete_unwrapped_top_level_acceleration_structure_instance_buffer = static_cast<pal_vk_top_level_acceleration_structure_instance_buffer *>(wrapped_top_level_acceleration_structure_instance_buffer);

	delete_unwrapped_top_level_acceleration_structure_instance_buffer->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks);

	delete_unwrapped_top_level_acceleration_structure_instance_buffer->~pal_vk_top_level_acceleration_structure_instance_buffer();
	pal_free(delete_unwrapped_top_level_acceleration_structure_instance_buffer);
}

void pal_vk_device::get_top_level_acceleration_structure_size(uint32_t top_level_acceleration_structure_instance_count, uint32_t *top_level_acceleration_structure_size, uint32_t *build_scratch_size, uint32_t *update_scratch_size) const
{
...
pal_top_level_acceleration_structure *pal_vk_device::create_top_level_acceleration_structure(uint32_t size) const
{
	void *new_unwrapped_top_level_acceleration_structure_base = pal_malloc(sizeof(pal_vk_top_level_acceleration_structure), alignof(pal_vk_top_level_acceleration_structure));
	assert(NULL != new_unwrapped_top_level_acceleration_structure_base);

	pal_vk_top_level_acceleration_structure *new_unwrapped_top_level_acceleration_structure = new (new_unwrapped_top_level_acceleration_structure_base) pal_vk_top_level_acceleration_structure{};
	new_unwrapped_top_level_acceleration_structure->init(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, this->m_top_level_acceleration_structure_memory_index, this->m_asset_allocator, size);
	return new_unwrapped_top_level_acceleration_structure;
}

void pal_vk_device::destroy_top_level_acceleration_structure(pal_top_level_acceleration_structure *wrapped_top_level_acceleration_structure) const
{
	assert(NULL != wrapped_top_level_acceleration_structure);
	pal_vk_top_level_acceleration_structure *delete_unwrapped_top_level_acceleration_structure = static_cast<pal_vk_top_level_acceleration_structure *>(wrapped_top_level_acceleration_structure);

	delete_unwrapped_top_level_acceleration_structure->uninit(this->m_pfn_get_device_proc_addr, this->m_device, this->m_allocation_callbacks, this->m_asset_allocator);

	delete_unwrapped_top_level_acceleration_structure->~pal_vk_top_level_acceleration_structure();
