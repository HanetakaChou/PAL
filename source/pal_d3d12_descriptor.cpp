...
pal_d3d12_descriptor_set_layout::pal_d3d12_descriptor_set_layout()
{
}

void pal_d3d12_descriptor_set_layout::init(uint32_t descriptor_set_binding_count, PAL_DESCRIPTOR_SET_LAYOUT_BINDING const *wrapped_descriptor_set_bindings)
{
	...
	for (uint32_t binding_index = 0U; binding_index < descriptor_set_binding_count; ++binding_index)
	{
		...
		{
			this->m_descriptor_layouts[binding_index].root_parameter_type = PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER;
		...
		{
			this->m_descriptor_layouts[binding_index].root_parameter_type = PAL_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
			this->m_descriptor_layouts[binding_index].root_parameter_shader_register = wrapped_descriptor_set_bindings[binding_index].binding;
		...
		{
			this->m_descriptor_layouts[binding_index].root_parameter_type = PAL_DESCRIPTOR_TYPE_SAMPLER;
			this->m_descriptor_layouts[binding_index].root_parameter_shader_register = wrapped_descriptor_set_bindings[binding_index].binding;
			this->m_descriptor_layouts[binding_index].root_descriptor_table_num_descriptors = wrapped_descriptor_set_bindings[binding_index].descriptor_count;
		}
		break;
		...
		{
			this->m_descriptor_layouts[binding_index].root_parameter_type = PAL_DESCRIPTOR_TYPE_STORAGE_IMAGE;
			this->m_descriptor_layouts[binding_index].root_parameter_shader_register = wrapped_descriptor_set_bindings[binding_index].binding;
			this->m_descriptor_layouts[binding_index].root_descriptor_table_num_descriptors = wrapped_descriptor_set_bindings[binding_index].descriptor_count;
		}
		break;
		...
		{
			this->m_descriptor_layouts[binding_index].root_parameter_type = PAL_DESCRIPTOR_TYPE_TOP_LEVEL_ACCELERATION_STRUCTURE;
			this->m_descriptor_layouts[binding_index].root_parameter_shader_register = wrapped_descriptor_set_bindings[binding_index].binding;
			this->m_descriptor_layouts[binding_index].root_descriptor_table_num_descriptors = wrapped_descriptor_set_bindings[binding_index].descriptor_count;
		}
		break;
		...
		{
			assert(false);
			this->m_descriptor_layouts[binding_index].root_parameter_type = static_cast<PAL_DESCRIPTOR_TYPE>(-1);
		}
		}
	}
}

void pal_d3d12_descriptor_set_layout::uninit()
{
}

uint32_t pal_d3d12_descriptor_set_layout::get_descriptor_layout_count() const
{
	return static_cast<uint32_t>(this->m_descriptor_layouts.size());
}

pal_d3d12_descriptor_layout const *pal_d3d12_descriptor_set_layout::get_descriptor_layouts() const
{
	pal_d3d12_descriptor_layout const *descriptor_layouts;
...
pal_d3d12_pipeline_layout::pal_d3d12_pipeline_layout() : m_root_signature(NULL)
{
}

void pal_d3d12_pipeline_layout::init(ID3D12Device *device, uint32_t descriptor_set_layout_count, pal_descriptor_set_layout const *const *descriptor_set_layouts)
{
	pal_vector<D3D12_ROOT_PARAMETER> root_parameters;
	pal_vector<D3D12_DESCRIPTOR_RANGE> root_descriptor_table_ranges;
	for (uint32_t set_index = 0U; set_index < descriptor_set_layout_count; ++set_index)
	{
		uint32_t binding_count = static_cast<pal_d3d12_descriptor_set_layout const *>(descriptor_set_layouts[set_index])->get_descriptor_layout_count();
		pal_d3d12_descriptor_layout const *descriptor_layouts = static_cast<pal_d3d12_descriptor_set_layout const *>(descriptor_set_layouts[set_index])->get_descriptor_layouts();
	...
	HRESULT const hr_serialize_root_signature = D3D12SerializeRootSignature(&root_signature_desc, D3D_ROOT_SIGNATURE_VERSION_1, &new_blob_with_root_signature, NULL);
...
void pal_d3d12_pipeline_layout::uninit()
{
	assert(NULL != this->m_root_signature);
...
pal_d3d12_pipeline_layout::~pal_d3d12_pipeline_layout()
{
	assert(NULL == this->m_root_signature);
}

ID3D12RootSignature *pal_d3d12_pipeline_layout::get_root_signature() const
{
	return this->m_root_signature;
}

pal_d3d12_descriptor_set::pal_d3d12_descriptor_set()
{
}

void pal_d3d12_descriptor_set::init(pal_d3d12_descriptor_allocator *descriptor_allocator, pal_descriptor_set_layout const *wrapped_descriptor_set_layout)
{
	assert(NULL != wrapped_descriptor_set_layout);
	pal_d3d12_descriptor_set_layout const *const unwrapped_descriptor_set_layout = static_cast<pal_d3d12_descriptor_set_layout const *>(wrapped_descriptor_set_layout);

	uint32_t const descriptor_layout_count = unwrapped_descriptor_set_layout->get_descriptor_layout_count();
	pal_d3d12_descriptor_layout const *unwrapped_descriptor_layouts = unwrapped_descriptor_set_layout->get_descriptor_layouts();
	...
	{
		pal_d3d12_descriptor_layout const &unwrapped_descriptor_layout = unwrapped_descriptor_layouts[descriptor_layout_index];
		pal_d3d12_descriptor &new_descriptor = this->m_descriptors[descriptor_layout_index];
		...
		{
			assert(false);
			new_descriptor.root_parameter_type = static_cast<PAL_DESCRIPTOR_TYPE>(-1);
		}
		}
	}
}

void pal_d3d12_descriptor_set::uninit(pal_d3d12_descriptor_allocator *descriptor_allocator)
{
	for (pal_d3d12_descriptor const &delete_descriptor : this->m_descriptors)
	{
...
void pal_d3d12_descriptor_set::write_descriptor(ID3D12Device *device, pal_d3d12_descriptor_allocator const *descriptor_allocator, PAL_DESCRIPTOR_TYPE wrapped_descriptor_type, uint32_t dst_binding, uint32_t dst_array_element, uint32_t src_descriptor_count, pal_upload_ring_buffer const *const *src_dynamic_uniform_buffers, uint32_t const *src_dynamic_uniform_buffers_range, pal_sampled_image const *const *src_sampled_images, pal_sampler const *const *src_samplers, pal_storage_image const *const *src_storage_images, pal_top_level_acceleration_structure const *const *src_top_level_acceleration_structures)
{
	...
	pal_d3d12_descriptor &destination_descriptor = this->m_descriptors[destination_descriptor_layout_index];
	...
	case PAL_DESCRIPTOR_TYPE_DYNAMIC_UNIFORM_BUFFER:
	{
		...
		assert(NULL != src_dynamic_uniform_buffers_range);
		...
		assert(NULL != src_dynamic_uniform_buffers_range[0]);
		destination_descriptor.root_constant_buffer_view.address_base = static_cast<pal_d3d12_upload_ring_buffer const *>(src_dynamic_uniform_buffers[0])->get_resource()->GetGPUVirtualAddress();
	}
	break;
	case PAL_DESCRIPTOR_TYPE_SAMPLED_IMAGE:
	{
		...
		for (uint32_t descriptor_index = 0U; descriptor_index < src_descriptor_count; ++descriptor_index)
		{
			...
			ID3D12Resource *const resource = static_cast<pal_d3d12_sampled_image const *>(src_sampled_images[descriptor_index])->get_resource();
			D3D12_SHADER_RESOURCE_VIEW_DESC const *const shader_resource_view_desc = static_cast<pal_d3d12_sampled_image const *>(src_sampled_images[descriptor_index])->get_shader_resource_view_desc();
	...
	case PAL_DESCRIPTOR_TYPE_SAMPLER:
	{
		...
		for (uint32_t descriptor_index = 0U; descriptor_index < src_descriptor_count; ++descriptor_index)
		{
			...
			D3D12_SAMPLER_DESC const *const sampler_desc = static_cast<pal_d3d12_sampler const *>(src_samplers[descriptor_index])->get_sampler_desc();
	...
	case PAL_DESCRIPTOR_TYPE_STORAGE_IMAGE:
	{
		...
		for (uint32_t descriptor_index = 0U; descriptor_index < src_descriptor_count; ++descriptor_index)
		{
			...
			ID3D12Resource *const resource = static_cast<pal_d3d12_storage_image const *>(src_storage_images[descriptor_index])->get_resource();
			D3D12_UNORDERED_ACCESS_VIEW_DESC const *const unordered_access_view_desc = static_cast<pal_d3d12_storage_image const *>(src_storage_images[descriptor_index])->get_unordered_access_view_desc();
	...
	case PAL_DESCRIPTOR_TYPE_TOP_LEVEL_ACCELERATION_STRUCTURE:
	{
		...
		for (uint32_t descriptor_index = 0U; descriptor_index < src_descriptor_count; ++descriptor_index)
		{
			...
			D3D12_SHADER_RESOURCE_VIEW_DESC const *const shader_resource_view_desc = static_cast<pal_d3d12_top_level_acceleration_structure const *>(src_top_level_acceleration_structures[descriptor_index])->get_shader_resource_view_desc();
...
uint32_t pal_d3d12_descriptor_set::get_descriptor_count() const
{
	return static_cast<uint32_t>(this->m_descriptors.size());
}

pal_d3d12_descriptor const *pal_d3d12_descriptor_set::get_descriptors() const
{
