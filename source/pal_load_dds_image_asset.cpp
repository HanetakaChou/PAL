...
extern bool pal_load_dds_image_asset_header_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER *image_asset_header, size_t *image_asset_data_offset);

extern bool pal_load_dds_image_asset_data_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER const *image_asset_header, size_t image_asset_data_offset, void *staging_buffer_base, size_t subresource_count, PAL_LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST const *subresource_memcpy_dests);
...
extern bool pal_load_dds_image_asset_data_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER const *image_asset_header, size_t image_asset_data_offset, void *staging_buffer_base, size_t subresource_count, PAL_LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST const *subresource_memcpy_dests)
{
    ...
    if (!pal_load_dds_image_asset_header_from_input_stream(input_stream, &image_asset_header_for_validate, &image_asset_data_offset_for_validate))
    {
        return false;
    }
    ...
    for (uint32_t planeSlice = 0; planeSlice < 1U; ++planeSlice)
    {
        ...
        for (uint32_t arraySlice = 0; arraySlice < image_asset_header->array_layers; ++arraySlice)
        {
            ...
            for (uint32_t mipSlice = 0; mipSlice < image_asset_header->mip_levels; ++mipSlice)
            {
                ...
                uint32_t dstSubresource = pal_load_image_asset_calculate_subresource_index(mipSlice, arraySlice, planeSlice, image_asset_header->mip_levels, image_asset_header->array_layers);
