...
pal_vk_concrete_color_attachment_image::pal_vk_concrete_color_attachment_image() : m_image(VK_NULL_HANDLE), m_device_memory(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void pal_vk_concrete_color_attachment_image::init(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, uint32_t color_transient_attachment_image_memory_index, uint32_t color_attachment_sampled_image_memory_index, PAL_COLOR_ATTACHMENT_IMAGE_FORMAT wrapped_color_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
...
void pal_vk_concrete_color_attachment_image::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
...
pal_vk_concrete_color_attachment_image::~pal_vk_concrete_color_attachment_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_device_memory);
	assert(VK_NULL_HANDLE == this->m_image_view);
...
VkImageView pal_vk_concrete_color_attachment_image::get_image_view() const
{
	return this->m_image_view;
}

pal_sampled_image const *pal_vk_concrete_color_attachment_image::get_sampled_image() const
{
	return static_cast<pal_vk_sampled_image const *>(this);
}

pal_vk_concrete_depth_stencil_attachment_image::pal_vk_concrete_depth_stencil_attachment_image() : m_image(VK_NULL_HANDLE), m_device_memory(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void pal_vk_concrete_depth_stencil_attachment_image::init(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, uint32_t depth_transient_attachment_image_memory_index, uint32_t depth_attachment_sampled_image_memory_index, uint32_t depth_stencil_transient_attachment_image_memory_index, uint32_t depth_stencil_attachment_sampled_image_memory_index, PAL_DEPTH_STENCIL_ATTACHMENT_IMAGE_FORMAT wrapped_depth_stencil_attachment_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	PFN_vkCreateImage const pfn_create_image = reinterpret_cast<PFN_vkCreateImage>(pfn_get_device_proc_addr(device, "vkCreateImage"));
	assert(NULL != pfn_create_image);
	PFN_vkGetImageMemoryRequirements const pfn_get_image_memory_requirements = reinterpret_cast<PFN_vkGetImageMemoryRequirements>(pfn_get_device_proc_addr(device, "vkGetImageMemoryRequirements"));
...
void pal_vk_concrete_depth_stencil_attachment_image::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
	PFN_vkDestroyImageView const pfn_destroy_image_view = reinterpret_cast<PFN_vkDestroyImageView>(pfn_get_device_proc_addr(device, "vkDestroyImageView"));
	assert(NULL != pfn_destroy_image_view);
	PFN_vkDestroyImage const pfn_destroy_image = reinterpret_cast<PFN_vkDestroyImage>(pfn_get_device_proc_addr(device, "vkDestroyImage"));
...
pal_vk_concrete_depth_stencil_attachment_image::~pal_vk_concrete_depth_stencil_attachment_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_device_memory);
	assert(VK_NULL_HANDLE == this->m_image_view);
...
VkImageView pal_vk_concrete_depth_stencil_attachment_image::get_image_view() const
{
	return this->m_image_view;
}

pal_sampled_image const *pal_vk_concrete_depth_stencil_attachment_image::get_sampled_image() const
{
	return static_cast<pal_vk_sampled_image const *>(this);
}

pal_vk_concrete_storage_image::pal_vk_concrete_storage_image() : m_image(VK_NULL_HANDLE), m_device_memory(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void pal_vk_concrete_storage_image::init(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, uint32_t storage_image_memory_index, PAL_STORAGE_IMAGE_FORMAT wrapped_storage_image_format, uint32_t width, uint32_t height, bool allow_sampled_image)
{
	PFN_vkCreateImage const pfn_create_image = reinterpret_cast<PFN_vkCreateImage>(pfn_get_device_proc_addr(device, "vkCreateImage"));
	assert(NULL != pfn_create_image);
	PFN_vkGetImageMemoryRequirements const pfn_get_image_memory_requirements = reinterpret_cast<PFN_vkGetImageMemoryRequirements>(pfn_get_device_proc_addr(device, "vkGetImageMemoryRequirements"));
	...
	uint32_t const memory_type_index = storage_image_memory_index;
...
void pal_vk_concrete_storage_image::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
	PFN_vkDestroyImageView const pfn_destroy_image_view = reinterpret_cast<PFN_vkDestroyImageView>(pfn_get_device_proc_addr(device, "vkDestroyImageView"));
	assert(NULL != pfn_destroy_image_view);
	PFN_vkDestroyImage const pfn_destroy_image = reinterpret_cast<PFN_vkDestroyImage>(pfn_get_device_proc_addr(device, "vkDestroyImage"));
...
pal_vk_concrete_storage_image::~pal_vk_concrete_storage_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_device_memory);
	assert(VK_NULL_HANDLE == this->m_image_view);
}
VkImage pal_vk_concrete_storage_image::get_image() const
{
	return this->m_image;
}

VkImageView pal_vk_concrete_storage_image::get_image_view() const
{
	return this->m_image_view;
}

pal_sampled_image const *pal_vk_concrete_storage_image::get_sampled_image() const
{
	return static_cast<pal_vk_sampled_image const *>(this);
}

pal_vk_asset_sampled_image::pal_vk_asset_sampled_image() : m_image(VK_NULL_HANDLE), m_allocation(VK_NULL_HANDLE), m_image_view(VK_NULL_HANDLE)
{
}

void pal_vk_asset_sampled_image::init(VmaAllocator asset_allocator, uint32_t asset_sampled_image_memory_index, PFN_vkCreateImageView pfn_create_image_view, VkDevice device, VkAllocationCallbacks const *allocation_callbacks, VkFormat format, uint32_t width, uint32_t height, uint32_t mip_levels)
{
	VkImageCreateInfo const image_create_info = {
		VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
		NULL,
	...
	allocation_create_info.memoryTypeBits = 1 << asset_sampled_image_memory_index;

	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_allocation);
	VkResult const res_vma_create_image = vmaCreateImage(asset_allocator, &image_create_info, &allocation_create_info, &this->m_image, &this->m_allocation, NULL);
...
void pal_vk_asset_sampled_image::uninit(VmaAllocator asset_allocator, PFN_vkDestroyImageView pfn_destroy_image_view, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
	assert(VK_NULL_HANDLE != this->m_image_view);
	pfn_destroy_image_view(device, this->m_image_view, allocation_callbacks);
	this->m_image_view = VK_NULL_HANDLE;
	...
	assert(VK_NULL_HANDLE != this->m_image);
	assert(VK_NULL_HANDLE != this->m_allocation);
	vmaDestroyImage(asset_allocator, this->m_image, this->m_allocation);
	this->m_image = VK_NULL_HANDLE;
	this->m_allocation = VK_NULL_HANDLE;
}

pal_vk_asset_sampled_image::~pal_vk_asset_sampled_image()
{
	assert(VK_NULL_HANDLE == this->m_image);
	assert(VK_NULL_HANDLE == this->m_allocation);
	assert(VK_NULL_HANDLE == this->m_image_view);
...
VkImage pal_vk_asset_sampled_image::get_image() const
{
	return this->m_image;
}

VkImageView pal_vk_asset_sampled_image::get_image_view() const
{
	return this->m_image_view;
}

pal_sampled_image const *pal_vk_asset_sampled_image::get_sampled_image() const
{
	return static_cast<pal_vk_sampled_image const *>(this);
}
