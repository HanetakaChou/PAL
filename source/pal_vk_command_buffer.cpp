...
pal_vk_graphics_command_buffer::pal_vk_graphics_command_buffer()
...
void pal_vk_graphics_command_buffer::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
...
VkCommandPool pal_vk_graphics_command_buffer::get_command_pool() const
{
	return this->m_command_pool;
}

VkCommandBuffer pal_vk_graphics_command_buffer::get_command_buffer() const
{
	return this->m_command_buffer;
}

VkSemaphore pal_vk_graphics_command_buffer::get_acquire_next_image_semaphore() const
{
	return this->m_acquire_next_image_semaphore;
}

VkSemaphore pal_vk_graphics_command_buffer::get_queue_submit_semaphore() const
{
	return this->m_queue_submit_semaphore;
}

void pal_vk_graphics_command_buffer::begin()
{
...
void pal_vk_graphics_command_buffer::acquire_asset_vertex_position_buffer(pal_asset_vertex_position_buffer *wrapped_asset_vertex_position_buffer)
{
	assert(NULL != wrapped_asset_vertex_position_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_vertex_position_buffer *>(wrapped_asset_vertex_position_buffer)->get_buffer();
	...
	if (this->m_has_dedicated_upload_queue)
	{
		if (this->m_upload_queue_family_index != this->m_graphics_queue_family_index)
		{
			...
			this->m_pfn_cmd_pipeline_barrier(this->m_command_buffer, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_VERTEX_INPUT_BIT, 0U, 0U, NULL, 1U, &acquire_barrier, 0U, NULL);
		}
		else
		{
			// do nothing
...
void pal_vk_graphics_command_buffer::acquire_asset_vertex_varying_buffer(pal_asset_vertex_varying_buffer *wrapped_asset_vertex_varying_buffer)
{
	assert(NULL != wrapped_asset_vertex_varying_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_vertex_varying_buffer *>(wrapped_asset_vertex_varying_buffer)->get_buffer();
...
void pal_vk_graphics_command_buffer::acquire_asset_index_buffer(pal_asset_index_buffer *wrapped_asset_index_buffer)
{
	assert(NULL != wrapped_asset_index_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_index_buffer *>(wrapped_asset_index_buffer)->get_buffer();
...
void pal_vk_graphics_command_buffer::acquire_asset_sampled_image(pal_asset_sampled_image *wrapped_asset_sampled_image, uint32_t dst_mip_level)
{
	assert(NULL != wrapped_asset_sampled_image);
	VkImage const asset_sampled_image = static_cast<pal_vk_asset_sampled_image *>(wrapped_asset_sampled_image)->get_image();
...
void pal_vk_graphics_command_buffer::acquire_asset_compacted_bottom_level_acceleration_structure(pal_asset_compacted_bottom_level_acceleration_structure *wrapped_asset_compacted_bottom_level_acceleration_structure)
{
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_compacted_bottom_level_acceleration_structure *>(wrapped_asset_compacted_bottom_level_acceleration_structure)->get_buffer();
...
void pal_vk_graphics_command_buffer::begin_debug_utils_label(char const *label_name)
{
#ifndef NDEBUG
...
void pal_vk_graphics_command_buffer::end_debug_utils_label()
{
#ifndef NDEBUG
...
void pal_vk_graphics_command_buffer::begin_render_pass(pal_render_pass const *pal_render_pass, pal_frame_buffer const *pal_frame_buffer, uint32_t width, uint32_t height, uint32_t color_clear_value_count, float const (*color_clear_values)[4], float const *depth_clear_value, uint8_t const *stencil_clear_value)
{
	...
	VkFramebuffer frame_buffer = static_cast<pal_vk_frame_buffer const *>(pal_frame_buffer)->get_frame_buffer();
...
void pal_vk_graphics_command_buffer::bind_graphics_pipeline(pal_graphics_pipeline const *wrapped_graphics_pipeline)
{
	assert(NULL != wrapped_graphics_pipeline);
	VkPipeline const graphics_pipeline = static_cast<pal_vk_graphics_pipeline const *>(wrapped_graphics_pipeline)->get_pipeline();

	this->m_pfn_cmd_bind_pipeline(this->m_command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, graphics_pipeline);
}

void pal_vk_graphics_command_buffer::set_view_port(uint32_t width, uint32_t height)
{
	// Vulkan Flip Y
...
void pal_vk_graphics_command_buffer::set_scissor(uint32_t width, uint32_t height)
{
	VkRect2D scissor = {{0, 0}, {width, height}};
	this->m_pfn_cmd_set_scissor(this->m_command_buffer, 0U, 1U, &scissor);
}

void pal_vk_graphics_command_buffer::bind_graphics_descriptor_sets(pal_pipeline_layout const *wrapped_pipeline_layout, uint32_t descriptor_set_count, pal_descriptor_set const *const *wrapped_descriptor_sets, uint32_t dynamic_offet_count, uint32_t const *dynamic_offsets)
{
	assert(NULL != wrapped_pipeline_layout);
	assert(NULL != wrapped_descriptor_sets);
	VkPipelineLayout const pipeline_layout = static_cast<pal_vk_pipeline_layout const *>(wrapped_pipeline_layout)->get_pipeline_layout();

	pal_vector<VkDescriptorSet> descriptor_sets(static_cast<size_t>(descriptor_set_count));
	for (uint32_t descriptor_set_index = 0U; descriptor_set_index < descriptor_set_count; ++descriptor_set_index)
	{
		...
		descriptor_sets[descriptor_set_index] = static_cast<pal_vk_descriptor_set const *>(wrapped_descriptor_sets[descriptor_set_index])->get_descriptor_set();
	}
...
void pal_vk_graphics_command_buffer::bind_vertex_buffers(uint32_t vertex_buffer_count, pal_vertex_buffer const *const *vertex_buffers)
{
	...
	for (uint32_t vertex_buffer_index = 0U; vertex_buffer_index < vertex_buffer_count; ++vertex_buffer_index)
	{
		...
		buffers[vertex_buffer_index] = static_cast<pal_vk_vertex_buffer const *>(vertex_buffers[vertex_buffer_index])->get_buffer();
...
void pal_vk_graphics_command_buffer::draw(uint32_t vertex_count, uint32_t instance_count)
{
	this->m_pfn_cmd_draw(this->m_command_buffer, vertex_count, instance_count, 0U, 0U);
}

void pal_vk_graphics_command_buffer::draw_index(pal_asset_index_buffer const *wrapped_index_buffer, PAL_GRAPHICS_PIPELINE_INDEX_TYPE wrapped_index_type, uint32_t index_count, uint32_t instance_count)
{
	assert(NULL != wrapped_index_buffer);
	VkBuffer const index_buffer = static_cast<pal_vk_asset_index_buffer const *>(wrapped_index_buffer)->get_buffer();
...
void pal_vk_graphics_command_buffer::end_render_pass()
{
	this->m_pfn_cmd_end_render_pass(this->m_command_buffer);
}

void pal_vk_graphics_command_buffer::compute_pass_load_storage_image(pal_storage_image const *wrapped_storage_image, PAL_STORAGE_IMAGE_LOAD_OPERATION load_operation)
{
	// load operation
	VkImage const load_image = static_cast<pal_vk_storage_image const *>(wrapped_storage_image)->get_image();
...
void pal_vk_graphics_command_buffer::bind_compute_pipeline(pal_compute_pipeline const *wrapped_compute_pipeline)
{
	assert(NULL != wrapped_compute_pipeline);
	VkPipeline const compute_pipeline = static_cast<pal_vk_compute_pipeline const *>(wrapped_compute_pipeline)->get_pipeline();

	this->m_pfn_cmd_bind_pipeline(this->m_command_buffer, VK_PIPELINE_BIND_POINT_COMPUTE, compute_pipeline);
}

void pal_vk_graphics_command_buffer::bind_compute_descriptor_sets(pal_pipeline_layout const *wrapped_pipeline_layout, uint32_t descriptor_set_count, pal_descriptor_set const *const *wrapped_descriptor_sets, uint32_t dynamic_offet_count, uint32_t const *dynamic_offsets)
{
	assert(NULL != wrapped_pipeline_layout);
	assert(NULL != wrapped_descriptor_sets);
	VkPipelineLayout const pipeline_layout = static_cast<pal_vk_pipeline_layout const *>(wrapped_pipeline_layout)->get_pipeline_layout();
...
void pal_vk_graphics_command_buffer::dispatch(uint32_t group_count_x, uint32_t group_count_y, uint32_t group_count_z)
{
	this->m_pfn_cmd_dispatch(this->m_command_buffer, group_count_x, group_count_y, group_count_z);
}

void pal_vk_graphics_command_buffer::compute_pass_store_storage_image(pal_storage_image const *wrapped_storage_image, PAL_STORAGE_IMAGE_STORE_OPERATION store_operation)
{
	// store operation
	VkImage const store_image = static_cast<pal_vk_storage_image const *>(wrapped_storage_image)->get_image();
...
void pal_vk_graphics_command_buffer::build_top_level_acceleration_structure(pal_scratch_buffer *wrapped_scratch_buffer, uint32_t scratch_buffer_offset, uint32_t top_level_acceleration_structure_instance_count, pal_top_level_acceleration_structure_instance_buffer *wrapped_top_level_acceleration_structure_instance_buffer, uint32_t top_level_acceleration_structure_start_instance_index, pal_top_level_acceleration_structure *wrapped_top_level_acceleration_structure)
{
	assert(NULL != wrapped_top_level_acceleration_structure);
	VkAccelerationStructureKHR const destination_acceleration_structure = static_cast<pal_vk_top_level_acceleration_structure *>(wrapped_top_level_acceleration_structure)->get_acceleration_structure();
	VkBuffer const destination_buffer = static_cast<pal_vk_top_level_acceleration_structure *>(wrapped_top_level_acceleration_structure)->get_buffer();

	assert(NULL != wrapped_top_level_acceleration_structure_instance_buffer);
	VkDeviceAddress const top_level_acceleration_structure_instance_buffer_device_memory_range_base = static_cast<pal_vk_top_level_acceleration_structure_instance_buffer *>(wrapped_top_level_acceleration_structure_instance_buffer)->get_device_memory_range_base();
		...
		{.instances =
			 {
				 ...
				 NULL,
				 VK_FALSE,
				 top_level_acceleration_structure_instance_buffer_device_memory_range_base + sizeof(VkAccelerationStructureInstanceKHR) * top_level_acceleration_structure_start_instance_index,
			 }}};

	assert(NULL != wrapped_scratch_buffer);
	VkDeviceAddress const scratch_buffer_device_memory_range_base = static_cast<pal_vk_scratch_buffer *>(wrapped_scratch_buffer)->get_device_memory_range_base();
...
void pal_vk_graphics_command_buffer::end()
{
...
pal_vk_upload_command_buffer::pal_vk_upload_command_buffer()
...
void pal_vk_upload_command_buffer::init(bool support_ray_tracing, bool has_dedicated_upload_queue, uint32_t graphics_queue_family_index, uint32_t upload_queue_family_index, PFN_vkGetInstanceProcAddr pfn_get_instance_proc_addr, VkInstance instance, PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
	this->m_support_ray_tracing = support_ray_tracing;
	this->m_has_dedicated_upload_queue = has_dedicated_upload_queue;
	this->m_graphics_queue_family_index = graphics_queue_family_index;
...
void pal_vk_upload_command_buffer::uninit(PFN_vkGetDeviceProcAddr pfn_get_device_proc_addr, VkDevice device, VkAllocationCallbacks const *allocation_callbacks)
{
	PFN_vkDestroyCommandPool const pfn_destroy_command_pool = reinterpret_cast<PFN_vkDestroyCommandPool>(pfn_get_device_proc_addr(device, "vkDestroyCommandPool"));
	assert(NULL != pfn_destroy_command_pool);
	PFN_vkFreeCommandBuffers const pfn_free_command_buffers = reinterpret_cast<PFN_vkFreeCommandBuffers>(pfn_get_device_proc_addr(device, "vkFreeCommandBuffers"));
...
VkCommandPool pal_vk_upload_command_buffer::get_upload_command_pool() const
{
	return this->m_upload_command_pool;
}

VkCommandBuffer pal_vk_upload_command_buffer::get_upload_command_buffer() const
{
	return this->m_upload_command_buffer;
}

VkCommandPool pal_vk_upload_command_buffer::get_graphics_command_pool() const
{
	return this->m_graphics_command_pool;
}

VkCommandBuffer pal_vk_upload_command_buffer::get_graphics_command_buffer() const
{
	return this->m_graphics_command_buffer;
}

VkSemaphore pal_vk_upload_command_buffer::get_upload_queue_submit_semaphore() const
{
	return this->m_upload_queue_submit_semaphore;
}

void pal_vk_upload_command_buffer::begin()
{
	if (this->m_has_dedicated_upload_queue)
	{
...
void pal_vk_upload_command_buffer::upload_from_staging_buffer_to_asset_vertex_position_buffer(pal_asset_vertex_position_buffer *wrapped_asset_vertex_position_buffer, uint64_t dst_offset, pal_staging_buffer *wrapped_staging_buffer, uint64_t src_offset, uint32_t src_size)
{
	assert(NULL != wrapped_asset_vertex_position_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_vertex_position_buffer *>(wrapped_asset_vertex_position_buffer)->get_buffer();

	assert(NULL != wrapped_staging_buffer);
	VkBuffer const staging_buffer = static_cast<pal_vk_staging_buffer *>(wrapped_staging_buffer)->get_buffer();
	...
	if (this->m_has_dedicated_upload_queue)
	{
		if (this->m_upload_queue_family_index != this->m_graphics_queue_family_index)
		{
			...
			this->m_pfn_cmd_copy_buffer(this->m_upload_command_buffer, staging_buffer, asset_buffer, 1U, &region);
	...
	else
	{
		...
		this->m_pfn_cmd_copy_buffer(this->m_graphics_command_buffer, staging_buffer, asset_buffer, 1U, &region);
...
void pal_vk_upload_command_buffer::upload_from_staging_buffer_to_asset_vertex_varying_buffer(pal_asset_vertex_varying_buffer *wrapped_asset_vertex_varying_buffer, uint64_t dst_offset, pal_staging_buffer *wrapped_staging_buffer, uint64_t src_offset, uint32_t src_size)
{
	assert(NULL != wrapped_asset_vertex_varying_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_vertex_varying_buffer *>(wrapped_asset_vertex_varying_buffer)->get_buffer();
...
void pal_vk_upload_command_buffer::upload_from_staging_buffer_to_asset_index_buffer(pal_asset_index_buffer *wrapped_asset_index_buffer, uint64_t dst_offset, pal_staging_buffer *wrapped_staging_buffer, uint64_t src_offset, uint32_t src_size)
{
	assert(NULL != wrapped_asset_index_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_index_buffer *>(wrapped_asset_index_buffer)->get_buffer();
...
void pal_vk_upload_command_buffer::upload_from_staging_buffer_to_asset_sampled_image(pal_asset_sampled_image *wrapped_asset_sampled_image, PAL_ASSET_IMAGE_FORMAT wrapped_asset_sampled_image_format, uint32_t asset_sampled_image_width, uint32_t asset_sampled_image_height, uint32_t dst_mip_level, pal_staging_buffer *wrapped_staging_buffer, uint64_t src_offset, uint32_t src_row_pitch, uint32_t src_row_count)
{
	...
	uint32_t const block_size = pal_get_format_block_size(wrapped_asset_sampled_image_format);
	...
	if (this->m_has_dedicated_upload_queue)
	{
		if (this->m_upload_queue_family_index != this->m_graphics_queue_family_index)
		{
			...
			this->m_pfn_cmd_copy_buffer_to_image(this->m_upload_command_buffer, staging_buffer, asset_sampled_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1U, &region);
	...
	else
	{
		...
		this->m_pfn_cmd_copy_buffer_to_image(this->m_graphics_command_buffer, staging_buffer, asset_sampled_image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1U, &region);
...
void pal_vk_upload_command_buffer::build_staging_non_compacted_bottom_level_acceleration_structure(pal_scratch_buffer *wrapped_scratch_buffer, uint32_t scratch_buffer_offset, pal_staging_non_compacted_bottom_level_acceleration_structure *wrapped_staging_non_compacted_bottom_level_acceleration_structure, uint32_t bottom_level_acceleration_structure_geometry_count, PAL_BOTTOM_LEVEL_ACCELERATION_STRUCTURE_GEOMETRY const *wrapped_bottom_level_acceleration_structure_geometries, pal_compacted_bottom_level_acceleration_structure_size_query_pool *wrapped_compacted_bottom_level_acceleration_structure_size_query_pool, uint32_t query_index)
{
	assert(NULL != wrapped_staging_non_compacted_bottom_level_acceleration_structure);
	VkAccelerationStructureKHR const destination_acceleration_structure = static_cast<pal_vk_staging_non_compacted_bottom_level_acceleration_structure *>(wrapped_staging_non_compacted_bottom_level_acceleration_structure)->get_acceleration_structure();

	assert(NULL != wrapped_bottom_level_acceleration_structure_geometries);
	pal_vector<VkBufferMemoryBarrier> vertex_position_buffer_load_barriers;
	pal_vector<VkBufferMemoryBarrier> index_buffer_load_barriers;
	...
	pal_vector<VkAccelerationStructureBuildRangeInfoKHR> acceleration_structure_build_range_infos;
	...
	for (uint32_t bottom_level_acceleration_structure_geometry_index = 0U; bottom_level_acceleration_structure_geometry_index < bottom_level_acceleration_structure_geometry_count; ++bottom_level_acceleration_structure_geometry_index)
	{
		...
		pal_vk_asset_vertex_position_buffer *const unwrapped_vertex_position_buffer = static_cast<pal_vk_asset_vertex_position_buffer *>(wrapped_bottom_level_acceleration_structure_geometry.vertex_position_buffer);
		...
		pal_vk_asset_index_buffer *const unwrapped_index_buffer = (PAL_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? static_cast<pal_vk_asset_index_buffer *>(wrapped_bottom_level_acceleration_structure_geometry.index_buffer) : NULL;
		...
		VkDeviceAddress const index_buffer_device_memory_range_base = (PAL_GRAPHICS_PIPELINE_INDEX_TYPE_NONE != wrapped_bottom_level_acceleration_structure_geometry.index_type) ? unwrapped_index_buffer->get_device_memory_range_base() : NULL;
	...
	VkQueryPool const query_pool = static_cast<pal_vk_compacted_bottom_level_acceleration_structure_size_query_pool *>(wrapped_compacted_bottom_level_acceleration_structure_size_query_pool)->get_query_pool();
	...
	VkAccelerationStructureBuildGeometryInfoKHR const acceleration_structure_build_geometry_info = {
		...
		NULL,
		{.deviceAddress = (scratch_buffer_device_memory_range_base + scratch_buffer_offset)}};
...
void pal_vk_upload_command_buffer::compact_bottom_level_acceleration_structure(pal_asset_compacted_bottom_level_acceleration_structure *wrapped_destination_asset_compacted_bottom_level_acceleration_structure, pal_staging_non_compacted_bottom_level_acceleration_structure *wrapped_source_staging_non_compacted_bottom_level_acceleration_structure)
{
	...
	VkAccelerationStructureKHR const source_acceleration_structure = static_cast<pal_vk_staging_non_compacted_bottom_level_acceleration_structure *>(wrapped_source_staging_non_compacted_bottom_level_acceleration_structure)->get_acceleration_structure();

	assert(NULL != wrapped_destination_asset_compacted_bottom_level_acceleration_structure);
	VkAccelerationStructureKHR const destination_acceleration_structure = static_cast<pal_vk_asset_compacted_bottom_level_acceleration_structure *>(wrapped_destination_asset_compacted_bottom_level_acceleration_structure)->get_acceleration_structure();
	VkBuffer const destination_buffer = static_cast<pal_vk_asset_compacted_bottom_level_acceleration_structure *>(wrapped_destination_asset_compacted_bottom_level_acceleration_structure)->get_buffer();
...
void pal_vk_upload_command_buffer::release_asset_vertex_position_buffer(pal_asset_vertex_position_buffer *wrapped_asset_vertex_position_buffer)
{
	assert(NULL != wrapped_asset_vertex_position_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_vertex_position_buffer *>(wrapped_asset_vertex_position_buffer)->get_buffer();
...
void pal_vk_upload_command_buffer::release_asset_vertex_varying_buffer(pal_asset_vertex_varying_buffer *wrapped_asset_vertex_varying_buffer)
{
	assert(NULL != wrapped_asset_vertex_varying_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_vertex_varying_buffer *>(wrapped_asset_vertex_varying_buffer)->get_buffer();
...
void pal_vk_upload_command_buffer::release_asset_index_buffer(pal_asset_index_buffer *wrapped_asset_index_buffer)
{
	assert(NULL != wrapped_asset_index_buffer);
	VkBuffer const asset_buffer = static_cast<pal_vk_asset_index_buffer *>(wrapped_asset_index_buffer)->get_buffer();
...
void pal_vk_upload_command_buffer::release_asset_sampled_image(pal_asset_sampled_image *wrapped_asset_sampled_image, uint32_t dst_mip_level)
{
	assert(NULL != wrapped_asset_sampled_image);
	VkImage const asset_sampled_image = static_cast<pal_vk_asset_sampled_image *>(wrapped_asset_sampled_image)->get_image();
...
void pal_vk_upload_command_buffer::release_asset_compacted_bottom_level_acceleration_structure(pal_asset_compacted_bottom_level_acceleration_structure *wrapped_asset_compacted_bottom_level_acceleration_structure)
{
...
void pal_vk_upload_command_buffer::end()
{
	if (this->m_has_dedicated_upload_queue)
	{
		if (this->m_upload_queue_family_index != this->m_graphics_queue_family_index)
