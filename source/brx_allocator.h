...
#ifndef _BRX_ALLOCATOR_H_
#define _BRX_ALLOCATOR_H_ 1
...
public:
    ...
    struct rebind
    {
        typedef brx_allocator<U> other;
    };

    brx_allocator() throw() {}
    brx_allocator(const brx_allocator &) throw() {}
    template <typename U>
    brx_allocator(const brx_allocator<U> &) throw() {}
    ...
    {
        pointer p = static_cast<pointer>(brx_malloc(sizeof(value_type) * n, alignof(value_type)));
        return p;
    }
...
inline bool operator==(const brx_allocator<T> &, const brx_allocator<U> &)
{
    return true;
}
