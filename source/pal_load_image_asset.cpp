...
#include <assert.h>
#include <algorithm>
#include "../include/pal_load_image_asset.h"
#include "pal_format.h"
#include "pal_align_up.h"
#include "pal_load_dds_image_asset.h"
#include "pal_load_pvr_image_asset.h"

extern "C" uint32_t pal_load_image_asset_calculate_subresource_index(uint32_t mip_level, uint32_t array_layer, uint32_t aspect_index, uint32_t mip_levels, uint32_t array_layers)
{
...
extern "C" size_t pal_load_image_asset_calculate_subresource_memcpy_dests(PAL_ASSET_IMAGE_FORMAT format, uint32_t width, uint32_t height, uint32_t depth, uint32_t mip_levels, uint32_t array_layers, size_t staging_buffer_base_offset, uint32_t staging_buffer_offset_alignment, uint32_t staging_buffer_row_pitch_alignment, uint32_t subresource_count, PAL_LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST *subresource_memcpy_dests)
{
    ...
    uint32_t const aspect_count = pal_get_format_aspect_count(format);
    uint32_t const block_size = pal_get_format_block_size(format);
    uint32_t const block_width = pal_get_format_block_width(format);
    uint32_t const block_height = pal_get_format_block_height(format);

    // vkspec: bufferOffset must be a multiple of 4
    staging_buffer_offset_alignment = pal_align_up(pal_align_up(staging_buffer_offset_alignment, 4U), block_size);
    ...
    for (uint32_t aspect_index = 0U; aspect_index < aspect_count; ++aspect_index)
    {
        for (uint32_t array_layer = 0U; array_layer < array_layers; ++array_layer)
        {
            ...
            for (uint32_t mipLevel = 0U; mipLevel < mip_levels; ++mipLevel)
            {
                ...
                uint32_t const output_row_pitch = pal_align_up(output_row_size, staging_buffer_row_pitch_alignment);
                uint32_t const output_slice_pitch = output_row_pitch * output_row_count;

                size_t const new_staging_buffer_offset = pal_align_up(staging_buffer_offset, static_cast<size_t>(staging_buffer_offset_alignment));
                total_bytes += (new_staging_buffer_offset - staging_buffer_offset);
                staging_buffer_offset = new_staging_buffer_offset;

                uint32_t const subresource_index = pal_load_image_asset_calculate_subresource_index(mipLevel, array_layer, aspect_index, mip_levels, array_layers);
...
//--------------------------------------------------------------------------------------
extern "C" bool pal_load_image_asset_header_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER *image_asset_header, size_t *image_asset_data_offset)
{
    ...
    {
    case DDS_MAGIC:
        return pal_load_dds_image_asset_header_from_input_stream(input_stream, image_asset_header, image_asset_data_offset);
    case Pvr_HeaderVersionV3:
        return pal_load_pvr_image_asset_header_from_input_stream(input_stream, image_asset_header, image_asset_data_offset);
    default:
        return false;
    }
}

extern "C" bool pal_load_image_asset_data_from_input_stream(pal_load_asset_input_stream *input_stream, PAL_LOAD_IMAGE_ASSET_HEADER const *image_asset_header, size_t image_asset_data_offset, void *staging_buffer_base, size_t subresource_count, PAL_LOAD_IMAGE_ASSET_SUBRESOURCE_MEMCPY_DEST const *subresource_memcpy_dests)
{
    if (-1 == input_stream->seek(0, LOAD_ASSET_INPUT_STREAM_SEEK_SET))
    {
        return false;
    ...
    {
    case DDS_MAGIC:
        return pal_load_dds_image_asset_data_from_input_stream(input_stream, image_asset_header, image_asset_data_offset, staging_buffer_base, subresource_count, subresource_memcpy_dests);
    case Pvr_HeaderVersionV3:
        return pal_load_pvr_image_asset_data_from_input_stream(input_stream, image_asset_header, image_asset_data_offset, staging_buffer_base, subresource_count, subresource_memcpy_dests);
    default:
        return false;
    }
}