...
pal_d3d12_frame_buffer::pal_d3d12_frame_buffer(pal_vector<ID3D12Resource *> &&render_target_resources, pal_vector<D3D12_CPU_DESCRIPTOR_HANDLE> &&render_target_view_descriptors, ID3D12Resource *depth_stencil_resource, pal_vector<D3D12_CPU_DESCRIPTOR_HANDLE> &&depth_stencil_view_descriptor) : m_render_target_resources(std::move(render_target_resources)), m_render_target_view_descriptors(std::move(render_target_view_descriptors)), m_depth_stencil_resource(depth_stencil_resource), m_depth_stencil_view_descriptor(std::move(depth_stencil_view_descriptor))
{
...
uint32_t pal_d3d12_frame_buffer::get_num_render_targets() const
{
	assert(this->m_render_target_resources.size() == this->m_render_target_view_descriptors.size());
	return static_cast<uint32_t>(this->m_render_target_resources.size());
}

ID3D12Resource *const *pal_d3d12_frame_buffer::get_render_target_view_resources() const
{
...
D3D12_CPU_DESCRIPTOR_HANDLE const *pal_d3d12_frame_buffer::get_render_target_view_descriptors() const
{
...
ID3D12Resource *pal_d3d12_frame_buffer::get_depth_stencil_resource() const
{
	return this->m_depth_stencil_resource;
}

D3D12_CPU_DESCRIPTOR_HANDLE const *pal_d3d12_frame_buffer::get_depth_stencil_view_descriptor() const
{
